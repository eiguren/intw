...
\textcolor{red}{Here, I wrote the transformation of the spinor is like multiplying this matrix on the 
vector column of spinor components, not the proper part of the inverse $\{R| \mathbf{\tau} \}^{-1}$, 
which is implemented in INTW now and seems to work. As Asier says, the spin is like another variable, 
...
&= |R| e^{-iR^{-1}\mathbf{b} \cdot \tau} \sum_{n_1}
  \braket{ \psi_{n_1 \mathbf{k}_s+R^{-1}\mathbf{b} } | \phi_{n \mathbf{k}_s+R^{-1}\mathbf{b}} }
...
{\it Note:} I think the steps above may be wrong if $\mathcal{S}$ has TR, because it would be antilinear. 
If we have spinors, the $\mathcal{S}$ has to be applied to the spins rotation in the potential derivative components.

\textcolor{blue}{TODO TR and spinor cases}
...
unitary transformation Eq.~\ref{eq:unitaryrot} (see last section).
...
\mathcal{S}^{-1} & \partial^\alpha_{s \mathbf{q}} V^{loc}(\mathbf{r})\mathcal{S} = \\
 &= -i |R| \sum_\beta \sum_{\mathbf{G}} R^{cart}_{\alpha\beta}(\mathbf{q_s} + \mathbf{G})_\beta
   ...
   e^{i (\mathbf{q_s} + \mathbf{G}) \cdot \mathbf{r} } = \\
 &=   |R| e^{i \mathbf{q_s} \cdot \mathbf{L} }   \sum_\beta R^{cart}_{\alpha\beta}  
...
M_{\alpha s \mathbf{q}}^{nm}(\mathbf{k}) & = 
  \braket{ \phi_{n \mathbf{k}_1+\mathbf{q}_s} 
   | \mathcal{S}^{-1} \partial^\alpha_{s \mathbf{q}} V^{loc} \mathcal{S} |
     \phi_{m \mathbf{k}_1} } = \\ 
&= |R|  e^{i \mathbf{q_s} \cdot \mathbf{L} }  \sum_{n_1m_1} 
...
If $\tilde R = I$ (unity operator), the coefficients will be $e^{i\alpha} \delta_{nm}$ for 
some global phase $\alpha$. 
If $\tilde R \neq I$, band off-diagonal non-zero elements can appear. 
...
\end{align}
\textcolor{red}{Repasar notas... parece que funciona para Si, pero hay que tener cuidado con las transpuestas 
