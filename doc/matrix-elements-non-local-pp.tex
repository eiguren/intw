...
\title{Electron-phonon matrix elements for non-local pseudo-potentials}
...
.\end{align}
Thus, due to the crystal periodicity and Bloch's theorem, the Fourier expansion of the wave function $\psi_{n\vec{k}}$ only has non-zero $\tilde{\psi}_{n\vec{k}}(\vec{G}_{SC})$ components that differ form $\vec{k}$ by a reciprocal space lattice vector $\vec{G}$. And, in fact,
\begin{align}
...
\end{equation}
and then $\verb|vpsi|(\vec{G})$ is obtained by making the Fourier transform. This is equivalent of doing the convolution in Fourier space of eq. \eqref{eq:dvpsi-combolution-1} and  \eqref{eq:dvpsi-combolution-1}, which is
\begin{equation}
\verb|vpsi|(\vec{G})
=
\sum_{\vec{G}'}
...
\tilde{u}_{m\vec{k}}(\vec{G}''')
.\end{equation}
\item Then, the variables $\verb|projec_1|(i')$ and $\verb|projec_2|(i')$ are multiplied by the $D_{si,si'}$ matrix to obtain $\verb|DKB_projec_1|(i)$ and $\verb|DKB_projec_2|(i)$:
\begin{equation}
\verb|DKB_projec_1|(i)
=
\sum_{i'}
...
\verb|projec_2|(i')
,\end{equation}
\item Next, the variable $\verb|dvnl_psi|$ is computed by multiplying ${\beta}_{s,i}$ by $\verb|DKB_projec_2|(i)$ and its derivative by $\verb|DKB_projec_1|(i)$:
\begin{equation}
\begin{array}{r@{}l}
\verb|dvnl_psi|(\vec{G})
=
...
\tilde{\beta}_{s,i}(\vec{k}+\vec{q}+\vec{G})\times \verb|DKB_projec_2|(i)
\\
&
-
i(\mathrm{k}_{\alpha}+\mathrm{q}_{\alpha}+\mathrm{G}_{\alpha})
\tilde{\beta}_{s,i}(\vec{k}+\vec{q}+\vec{G})\times \verb|DKB_projec_1|(i)
\Big]
\end{array}
.\end{equation}
