program imaginary_selfen
  ...
  complex(dp), allocatable :: aep_mat_el(:,:,:,:,:,:,:),aep_mat_el_3(:,:,:,:,:,:,:),ep_mat_el(:,:,:,:,:,:)
  ...
  complex(dp),allocatable :: g_Re(:,:,:,:),g_wannier(:,:,:),g_Rp(:,:,:,:)
...
!  allocate (aep_mat_el(nqmesh,nk1*nk2*nk3,4,4,nspin,nspin,3*nat))
...
!  allocate (ep_mat_el(nk1*nk2*nk3,4,4,nspin,nspin,3*nat))
  !
  !================================================================================
                 ...
                 do ibnd=1,num_bands
                    do jbnd=1,num_bands
                       ...
                       do is=1,nspin
                          do js=1,nspin
                             !
                             aep_mat_el_2(iq,ik,ibnd,jbnd,imode)=aep_mat_el_2(iq,ik,ibnd,jbnd,imode)+ &
                                                                 aep_mat_el(iq,ik,ibnd,jbnd,is,js,imode)
                             !
                          enddo !js
                       enddo !is
                       !
...
!         do jbnd=1,num_wann
!            do imode=1,3*nat
!               write(1789,'(a,i3,a,i3,a,i3,a,i3,a,i3)')'q= ',iq,' k= ',ikpt_k,' ib= ',ibnd,' jb= ',jbnd,' imode= ',imode
!               write(1789,'(100G16.6)') aW_rot_ep_mat_el(iq,ikpt_k,ibnd,jbnd,imode)
!               write(1789,*) ' '
!            enddo !imode
!         enddo !jbnd
!      enddo !ibnd
...
!enddo !iq
     ...
     !
     deallocate(aep_mat_el,aep_mat_el_2,ep_mat_el,aW_rot_ep_mat_el,W_rot_ep_mat_el)
     !     
     !==============================================================================================
  ...
  allocate(g_me(num_wann,num_wann,3*nat))
  !
  !==========================================================================
...
!   do jbnd=1,num_wann
!      do imode=1,3*nat
!         write(1799,'(a,i3,a,i3,a,i3,a,i3,a,i3)')'q= ',iq,' k= ',ik,' ib= ',ibnd,' jb= ',jbnd,' imode= ',imode
!         write(1799,'(100G16.6)') g_wannier(ibnd,jbnd,imode)
!         write(1799,*) ' '
!      enddo !imode
!   enddo !jbnd
!enddo !ibnd
...
!     trshld=1.6087
  ...
  !
  allocate(sumg1(nkfmesh),sumg2(nkfmesh))
  !
  ...
  !
  !$omp parallel default(none) &
  !$omp shared(nkfmesh,kfmesh,nkf1,nkf2,nkf3,qpoint,eigen,cmplx_0,g_Re,nrr_k,irr_k,ndegen_k,wqv,ef,eps_5,eps_6,nspin) &
  !$omp shared(sumg1,sumg2,U_el,U_phon,imode,num_wann,iq,nat,tpi,cmplx_i) &
  !$omp private(ik,kpoint,i,j,k,kpoint_in_1bz,kqpoint_in_1bz,GKQ_bz,ikpt_k,ikpt_kq,ibnd,jbnd,ekk,ekq,g_wannier,g_elph,g2) 
  !
  !$omp do
  !
  do ik=1,nkfmesh
...
!        do jbnd=1,num_wann
        do jbnd=3,6
           ...
           if (ekk.lt.ef.and.ekq.gt.ef) then
              !
              g2=(abs(g_elph(ibnd,jbnd,1))**2.d0)
              !
              sumg1(ik)=sumg1(ik)+g2
              !
              sumg2(ik)=sumg2(ik)+g2/(ekk-ekq)
              !
!write(*,*) 'jbnd = ',jbnd,' ibnd = ',ibnd
!write(*,*) g2
              !
...
!stop
  ...
  !$omp end parallel
  !
  allocate(sumg1_tex(-nkf1:nkf1,-nkf2:nkf2),sumg2_tex(-nkf1:nkf1,-nkf2:nkf2))
  sumg1_tex(:,:)=0.0d0
  sumg2_tex(:,:)=0.0d0
  !
  do ik=1,nkfmesh
     ...
     call find_k_1BZ_and_G(kpoint,nkf1,nkf2,nkf3,i,j,k,kpoint_in_1bz,GKQ_bz)
     !
     sumg1_tex((i-1)-nkf1,(j-1)-nkf2)=sumg1(ik)
     sumg1_tex((i-1)-nkf1,j-1)=sumg1(ik)
     sumg1_tex(i-1,(j-1)-nkf2)=sumg1(ik)
     sumg1_tex(i-1,j-1)=sumg1(ik)
     !
     sumg2_tex((i-1)-nkf1,(j-1)-nkf2)=sumg2(ik)
     sumg2_tex((i-1)-nkf1,j-1)=sumg2(ik)
     sumg2_tex(i-1,(j-1)-nkf2)=sumg2(ik)
  ...
  do ik=-nkf1,nkf1-1
     !
     sumg1_tex(ik,nkf2)=sumg1_tex(ik,0)
     sumg2_tex(ik,nkf2)=sumg2_tex(ik,0)
     !
  enddo !ik
  !
  do ik=-nkf2,nkf2-1
     !
     sumg1_tex(nkf1,ik)=sumg1_tex(0,ik)
     sumg2_tex(nkf1,ik)=sumg2_tex(0,ik)
     !
  enddo !ik
  !
  sumg1_tex(nkf1,nkf2)=sumg1_tex(0,0)
  sumg2_tex(nkf1,nkf2)=sumg2_tex(0,0)
  !
  open(unit=11,file=trim(trim(mesh_dir)//trim('g_tex.dat_no_spin')))
  open(unit=22,file=trim(trim(mesh_dir)//trim('g_zati_w_tex.dat_no_spin')))
  !
  do ikx=-nkf1,nkf1
     do iky=-nkf2,nkf2
        !
