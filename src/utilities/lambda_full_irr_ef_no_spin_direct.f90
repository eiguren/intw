program eliashberg_full_no_spin
  ...
  real(dp) :: ef,trshld,trshld1,trshld2,w_max,dkvec(3)
  ...
  !Filer of q-ak and k-ak
  integer :: iq_cont,iq_2,nqfmesh_2,ik_cont,ik_2,nkfmesh_2
  integer,allocatable :: list_iq(:,:),list_ik(:,:)

  !local/aux variables
  real(dp)                 :: sigma_e,qmoduloa,dosef
  ...
  real(dp),parameter :: rytoev=13.605698066d0 , amutory=911.44424213227238468637d0
!  real(dp),parameter :: rytoev=1.d0 , amutory=1.d0
...
!     ef=12.5709d0 !Cu bulk
!     ef=7.7878d0 !Al bulk
!     ef=6.1957d0 !Pb bulk
!     ef=6.1142d0 !Pb bulk SOC
!     ef=6.3440d0 !Pb bulk
!     ef=5.2067d0 !Tl bulk
     ef=-2.2117d0+1.5d0
  else
...
!     ef=5.2067d0 !Tl bulk
  ...
  !
  !==============================================================================================
  ! Calculation of the  EPC strength/mass enhancement parameter lambda for a fixed energy level
  !
  ...
  ! We know:
  !
  ! lambda=Sum_k_q_m_n_nu[|g(q,k,m,n,nu)|^2*Delta[E(k,n)-E(k+q,m)+/-wqv]*Delta[E(k,n)-Ef]/wqv]
  !
  ...
  !
  allocate(list_ik(nkf_irr*num_wann,2))
  allocate(list_iq(nqfmesh*num_wann,2))
  !
!  sigma_e=0.136056981d0*2.d0
  sigma_e=0.020d0
  !
  list_ik(:,:)=0
  ik_cont=0
  ...
  do ik=1,nkf_irr
     ...
     !
     ! We choose the states (k,n) take part in the Delta[E(k,n)-Ef]
     !
     do ibnd=1,num_wann
        !
        ! We define the eigenvalues of the state (k,n)
        !
        ekk=eigen(ibnd,ikpt_k)
        !
        ! We create the list of terms we are interested in
        !
        if (abs(ekk-ef).gt.4.d0*sigma_e) then
           !
        else
           !
           ik_cont=ik_cont+1
           list_ik(ik_cont,1)=ik
           list_ik(ik_cont,2)=ibnd
           dosef=dosef+weightf_irr(ik)*exp(-((ekk-ef)**2.d0)/(sigma_e**2.d0))/(sigma_e*sqrt(pi))
           !
        endif !abs(ekk-ef)
        !
     enddo !ibnd
  enddo !ik
  !
  nkfmesh_2=ik_cont !how many states (k,n) we take into account
  !
  do ik_2=1,nkfmesh_2
     !
     if ((ik_2/10)*10==ik_2) then
        write(*,*) 'ik = ',ik_2,'/',nkfmesh_2
     endif
     !
     ik=list_ik(ik_2,1)
     jbnd=list_ik(ik_2,2)
...
!           if (abs(ekk-ekq).gt.w_max) then
     ...
     do iq_2=1,nqfmesh_2
        !
        if ((iq_2/50)*50==iq_2) then
           write(*,*) 'iq = ',iq_2,'/',nqfmesh_2,' ik = ',ik_2,'/',nkfmesh_2
        endif
        !
        ...
        do imode=1,3*nat
           ...
           if (abs(ekk-ekq+wqv).gt.4*sigma_e) then
              ...
              !
              lambda=lambda+weightf_irr(ik)*(g2/wqv)*(exp(-((ekk-ef)/sigma_e)**2.d0)/(sigma_e*sqrt(pi)))* &
                      (exp(-((ekk-ekq+wqv)/sigma_e)**2.d0)/(sigma_e*sqrt(pi)))
           endif
           !
           if (abs(ekk-ekq-wqv).gt.4*sigma_e) then
              !
  ...
  !
  lambda=lambda/nqfmesh/dosef
  write(*,*) lambda
  !
  ! We write our results
  !
  write(1314,'(a,100f16.8)') 'lambda(ef)= ',lambda
  !
  deallocate(list_iq)
  deallocate(list_ik)
