program phonon_linewidths_nuq 
  ...
  real(dp)                 :: qpoint(3),qpoint_in_1bz(3), qpoint_1bz(3), qpoint_rot(3), qpoint_cart(3)
  integer                  :: ikpt_q
  ...
  real(dp) :: trshld,trshld1,trshld2,w_max,dqvec(3),ef
  real(dp) :: ekq,ekk,wqv
  real(dp),allocatable :: phonlw(:,:)
  ...
  !Filer of k-ak
  integer :: ik_cont,ik_2,nkfmesh_2
  integer,allocatable :: list_ik(:,:)
  ...
  integer                  :: ii,jj,kk
  integer                  :: jr,kr
  integer                  :: ig,ibnd,jbnd,is,js,iibnd,jjbnd,ifs,nqq,nfs
  integer                  :: switch
...
!THIS IS FOR AVERAGE PROGRAM
  ...
  !
  if (lspinorb) then
     trshld1=-2.57509105d0 !TlSi downbands
     trshld2=-2.39934425d0 !TlSi upbands
!     trshld=1.9594 !C graph 
  else
     trshld1=-2.57838555d0 !TlSi downbands
     trshld2=-2.47036625d0 !TlSi upbands
!     trshld=1.9594 !C graph
  endif
  !
  ef=0.350d0
  ...
  open(unit=1001,file=trim(trim(mesh_dir)//trim('phonlw.dat')))
  open(unit=1002,file=trim(trim(mesh_dir)//trim('phonlw.bin')),form='unformatted')
  !
  !==============================================================================================
  ! Calculation of the phonon linewidth gamma(q,nu)
  ...
  ! We know:
  !
  ! gamma(q,nu)=Pi*Sum_k_m_n[|g(q,k,m,n,nu)|^2*Delta[E(k,n)-E(k+q,m)+/-wqv]*(f(E(k,n))-f(E(k+q,m)))]
  !
  ...
  !
  allocate(list_ik(nkfmesh*num_wann*num_wann*3*nat,5))
  !
!  sigma_e=0.010d0
  sigma_e=0.0015d0
  !
...
!           sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2),eigen(:,iqpt)
     ...
     if (iq.lt.168) then
        !
        write(1314,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2),abs(omega(:,iqpt))
        !
     elseif (iq.gt.167.and.iq.lt.251) then
        !
        write(1314,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           1.d0-sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2-1.d0/3.d0),abs(omega(:,iqpt))
        !
     elseif (ik.gt.250) then
        !
        write(1314,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           1.d0+sqrt(1.d0/3.d0)-sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2),abs(omega(:,iqpt))
        !
     endif
     !
     if (iwant) then
              ...
              do jbnd=1,num_wann
                 ...
                 if (ekk.lt.ef.and.ekq.gt.ef) then
                    ...
                    if (abs(ekk-ekq+wqv).gt.(4*sigma_e)) then
                       ...
                       !
                       ik_cont=ik_cont+1
                       list_ik(ik_cont,1)=ikpt_k
                       list_ik(ik_cont,2)=ikpt_kq
                       list_ik(ik_cont,3)=ibnd
                       list_ik(ik_cont,4)=jbnd
                       list_ik(ik_cont,5)=imode
                       !
                    endif !abs(ekk-ekq+wqv)
                    !
                 endif ! ekk<ef, ekq>ef
                 !
              enddo !jbnd
     ...
     do ik_2=1,nkfmesh_2
        !
        if ((ik_2/1000)*1000==ik_2) then
           write(*,*) 'ik = ',ik_2,'/',nkfmesh_2,' iq = ',iq,'/',nqq
        endif
        !
        ikpt_k=list_ik(ik_2,1)
        ikpt_kq=list_ik(ik_2,2)
        ibnd=list_ik(ik_2,3)
        jbnd=list_ik(ik_2,4)
        imode=list_ik(ik_2,5)
        !
        kpoint(:)=kfmesh(:,ikpt_k)
        kqpoint(:)=kfmesh(:,ikpt_kq)
        if (ibnd.lt.2) ekq=eigen(ibnd,ikpt_kq)+trshld1
        if (ibnd.gt.1) ekq=eigen(ibnd,ikpt_kq)+trshld2
...
!           g2=weight_ph(wqv)*(abs(g_me(imode))**2.d0)/(2.d0*wqv)
           g2=(abs(g_me(imode))**2.d0)/(2.d0*wqv)
           !
        endif !wqv > 0
        !
        phonlw(iq,imode)=phonlw(iq,imode)+2*pi*g2*exp(-((ekk-ekq+wqv)/sigma_e)**2.d0)/(sigma_e*sqrt(pi))
        !
     enddo !ik_2
     !
     phonlw(iq,:)=phonlw(iq,:)/nkfmesh
     !
     ! We write our results
     !
     write(1001,'(i5,100f16.8)') iq,phonlw(iq,:)
     write(1002) phonlw(iq,:)
     !
  enddo !iq
  !
  close(1001)
...
end program phonon_linewidths_nuq
