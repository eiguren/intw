program imaginary_selfen
  ...
  complex(dp), allocatable :: aep_mat_el_2(:,:,:,:)
...
!  file=trim(trim(mesh_dir)//trim('EP_DATA_IND/')//trim(ep_mat_file)//trim('_K')), &
  ...
  g_kq(:,:,:,:)=aep_mat_el_2(:,:,:,:)
  !
  deallocate(aep_mat_el,aep_mat_el_2,ep_mat_el)
  !     
  !====================================================================================
...
!        do jbnd=1,3
           ...
           if (ekk.lt.ef.and.ekq.gt.ef) then
              !
              g2=(abs(g_elph(ik,ibnd,jbnd,1))**2.d0)
              !
  ...
  !
  open(unit=11,file=trim(trim(mesh_dir)//trim('g_tex.dat_K')))
  open(unit=22,file=trim(trim(mesh_dir)//trim('g_zati_w_tex.dat_K')))
