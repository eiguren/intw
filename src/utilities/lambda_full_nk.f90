program lambda_full_nk
  ...
  real(dp) :: trshld,w_min,w_max,d_w,lambda
  real(dp),allocatable :: eliash_fplus(:),eliash_fminus(:)
  real(dp),allocatable :: eliash_fplus_so(:,:,:),eliash_fminus_so(:,:,:)
  integer :: lambda_so_unit,lambda_unit
  real(dp),allocatable :: lambda_so(:)
  ...
  if (lspinorb) then
     ...
     allocate(lambda_so(nspin*nspin))
     lambda_so(:)=0.0d0
     !
  endif
  !
  ...
  if (lspinorb) then
     !
     lambda_so_unit=find_free_unit()
     open(unit=lambda_so_unit,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/lambda_so.dat')))
     !
  endif !lspinorb
  !
  lambda_unit=find_free_unit()
  open(unit=lambda_unit,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/lambda.dat')))
  !
  do ik=1,nkk
     !
     ...
     if (lspinorb) then
        ...
        do ifs=1,nfs
           ...
           !
           if (ik<10) then
              open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_plus_so_')//trim(ik_loc1)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
              open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_minus_so_')//trim(ik_loc1)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<100) then
              open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_plus_so_')//trim(ik_loc2)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
              open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_minus_so_')//trim(ik_loc2)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<1000) then
              open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_plus_so_')//trim(ik_loc3)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
              open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_minus_so_')//trim(ik_loc3)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<10000) then
              open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_plus_so_')//trim(ik_loc4)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
              open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_minus_so_')//trim(ik_loc4)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           endif
           !
           do ipts=1,wpts
              !
              if (ifs==1) then
                 !
                 read(10*ik+1) eliash_fplus_so(ipts,1,1)
                 read(10*ik+2) eliash_fminus_so(ipts,1,1)
                 !
              elseif (ifs==2) then
                 !
                 read(10*ik+1) eliash_fplus_so(ipts,1,2)
                 read(10*ik+2) eliash_fminus_so(ipts,1,2)
                 !
              elseif (ifs==3) then
                 !
                 read(10*ik+1) eliash_fplus_so(ipts,2,1)
                 read(10*ik+2) eliash_fminus_so(ipts,2,1)
                 !
              elseif (ifs==4) then
                 !
                 read(10*ik+1) eliash_fplus_so(ipts,2,2)
                 read(10*ik+2) eliash_fminus_so(ipts,2,2)
                 !
              endif
              !
           enddo !ipts
        ...
        enddo !ifs
        !
        call lambda_calculate_full_t0_so(wpts,w_min,w_max,d_w,eliash_fplus_so,eliash_fminus_so,lambda_so)
        write(lambda_so_unit,'(i4,4f10.3)') ik,(lambda_so(ifs),ifs=1,nfs)
        !
     endif !lspinorb
     !
     if (ik<10) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_plus_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_minus_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
     elseif (ik<100) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_plus_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_minus_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
     elseif (ik<1000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_plus_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_minus_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
     elseif (ik<10000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_plus_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('elf_down_1000_1000_s1down/eliash_minus_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
     endif
     !
     do ipts=1,wpts
        !
        read(10*ik+1) eliash_fplus(ipts)
        read(10*ik+2) eliash_fminus(ipts)
        !
     enddo !ipts
     !
     close(10*ik+1)
     close(10*ik+2)
     !
     call lambda_calculate_full_t0(wpts,w_min,w_max,d_w,eliash_fplus,eliash_fminus,lambda)
     write(lambda_unit,'(i4,4f10.3)') ik,lambda
     !
  enddo !nkk
  !
  if (lspinorb) close(unit=lambda_so_unit)
  close(unit=lambda_unit)

end program lambda_full_nk
