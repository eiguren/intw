program ep_melements
  ...
  MPI_nbands_initial = nint(0.5+1.0*nbands/nNodes)*myNode + 1
  MPI_nbands_final   = nint(0.5+1.0*nbands/nNodes)*(myNode + 1)
  if (myNode+1==nNodes) MPI_nbands_final=nbands
  write(stdout,'(a,i2,a,i4,a,i4,a)') '|      - Node: ',myNode,' bands: ',MPI_nbands_initial,'-',MPI_nbands_final,'                 |'
  !
  if (calc_epmat) then
    !
    !
    ...
    do imode=1,411
      ...
      do jband=MPI_nbands_initial,MPI_nbands_final
        ...
        write(stdout,"(i2,a8,i3,a1,i3,x,a5,i7,a1,i7)") &
              myNode, " Mode: ", imode,"/",3*nat, "wfc: ", jband,"/",(MPI_nbands_final)
    ...
    do iNode=0,nNodes-1
      MPI_nbands_initial = nint(0.5+1.0*nbands/nNodes)*iNode + 1
      MPI_nbands_final   = nint(0.5+1.0*nbands/nNodes)*(iNode + 1)
      if (iNode+1==nNodes) MPI_nbands_final=nbands
      nSize=size(ep_mat_el(MPI_nbands_initial:MPI_nbands_final,:))
      call MPI_Bcast( ep_mat_el(MPI_nbands_initial:MPI_nbands_final,:), nSize, MPI_double_precision, iNode, MPI_COMM_WORLD, MPIerror )
    enddo
    !
    if (myNode==master) then
      !
