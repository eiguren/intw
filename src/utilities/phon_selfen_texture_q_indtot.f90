program imaginary_selfen
  ...
  complex(dp) :: g1
  real(dp) :: g2
  complex(dp),allocatable :: g_kq(:,:,:,:),g_elph(:,:,:,:),pol_gkq(:,:),pol_tex(:,:,:)
...
!  allocate (ep_mat_el(nk1*nk2*nk3,4,4,1,1,3*nat))
  ...
  open(unit=ep_unit,&
  file=trim(trim(mesh_dir)//trim(ep_mat_file)//trim('_K')), &
...
!  file=trim(trim(mesh_dir)//trim(ep_mat_file)//trim('_K_ext_berria')), &
  iostat=ierr,form='unformatted',status='unknown',access='direct',recl=record_lengh)
  !
...
!                                                      aep_mat_el_ind(ik,ibnd,jbnd,2,1,imode)+aep_mat_el_ind(ik,ibnd,jbnd,2,2,imode))
                 !
                 aep_mat_el_2_ind(ik,ibnd,jbnd,imode)=aep_mat_el_ind(ik,ibnd,jbnd,1,1,imode)+aep_mat_el_ind(ik,ibnd,jbnd,1,2,imode)+ &
                                                      aep_mat_el_ind(ik,ibnd,jbnd,2,1,imode)+aep_mat_el_ind(ik,ibnd,jbnd,2,2,imode)
                 !
              else
                 !
...
!                 aep_mat_el_2_ind(ik,ibnd,jbnd,imode)=aep_mat_el_2(ik,ibnd,jbnd,imode)-aep_mat_el_ind(ik,ibnd,jbnd,1,1,imode)
                 aep_mat_el_2_ind(ik,ibnd,jbnd,imode)=aep_mat_el_ind(ik,ibnd,jbnd,1,1,imode)
                 !
              endif
              !
           enddo !jbnd
...
write(*,*)''
        ...
        do ibnd=1,num_bands
           do jbnd=1,num_bands
              !
              g2=weight_ph(wqv)*(abs(g_elph(ik,ibnd,jbnd,imode))**2.d0)/(2.d0*wqv)
              !
write(*,*)'ibnd= ',ibnd,' jbnd= ',jbnd
write(*,*) sqrt(g2)
write(*,*)''
              !
           enddo !jbnd
...
!              g2=2*real(conjg(g_elph(ik,ibnd,jbnd,2))*g_elph_ind(ik,ibnd,jbnd,2))
!              g2=2*real(conjg(g_elph(ik,ibnd,jbnd,1))*g_elph(ik,ibnd,jbnd,1))
              !
...
!              g2=2*real(conjg(g_elph(ik,ibnd,jbnd,1))*g_elph(ik,ibnd,jbnd,1))
  ...
  !
  open(unit=11,file=trim(trim(mesh_dir)//trim('xi_tex.dat_mode1')))
  open(unit=22,file=trim(trim(mesh_dir)//trim('xi_zati_w_tex.dat_mode1')))
  !
  do ikx=-nk1,nk1
     do iky=-nk2,nk2
        !
