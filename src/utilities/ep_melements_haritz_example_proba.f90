program ep_melements
  ...
  real(kind=dp), external :: omp_get_wtime
  ...
  allocate( all_wfc_k(nG_max,nbands), wfc_kq(nG_max,num_bands) )
  list_iG  = 0
  eig      = ZERO
  all_wfc_k  = cmplx_0
  wfc_kq = cmplx_0
    ...
    do jband=1,num_bands
      ...
      do imode=1,3*nat
        do iband=1,num_bands
          !
          call matrix_element(wfc_kq(:,iband),dvpsi(:,1,1,1,imode),ep_mat_el(iband,jband,imode))
          !
        enddo ! imode
      enddo ! iband
      !
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine calculate_dvloc_psi(nat,nG_max,nbands,igk,wfc_k_g,dvpsi)
    ...
    !
    !$OMP PARALLEL PRIVATE(dv_local,aux)
    !$OMP DO
    do imode=1,3*nat
      !
