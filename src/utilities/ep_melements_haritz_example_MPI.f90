program ep_melements
  ...
  ! allocate( all_wfc_k(nG_max,nbands), wfc_kq(nG_max,nbands_selected) )
  ...
  allocate( wfc_k(nG_max,nbands_selected,1) )
    ...
    if (myNode==master) then
      ...
      do imode=1,3*nat
        do jband=1,nbands_selected
          do iband=1,nbands_selected
            print"(a6,3i5,4x,2f15.10,4x,f15.10)", "haritz", iband, nbands_initial+jband-1, imode, ep_mat_el(iband,jband,imode), abs(ep_mat_el(iband,jband,imode))
          enddo
        enddo
      enddo
      !
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine calculate_dvloc_psi(nat,nG_max,nbands,igk,wfc_k_g,dvpsi)
    ...
    dvfile = trim(mesh_dir) // trim(ph_dir) // trim(dvscf_dir) // trim(dvscf_name)
    open( unit=read_unit, file=trim(dvfile), iostat=ios, form='unformatted', &
    ...
    do imode=1,3*nat
      ...
      ! dv_local = cmplx_0
      rec= nspin*(imode-1) + ispin
      read(unit=read_unit,rec=rec,iostat=ios) dv_local
      !
