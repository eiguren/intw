program selfen_direct_nk
  ...
  integer :: npts,epts,ipts
  real(dp) :: temp ! temperature of the system
  real(dp) :: trshld,ef,e_min,e_max,d_e,dkvec(3),w_max
  real(dp) :: ekq,ekk,delta_eplus,delta_eminus,wqv,e
  real(dp),allocatable :: imselfen_w(:)
  ...
  integer                  :: ii,jj,kk
  integer                  :: jr,kr
  integer                  :: ig,ibnd,jbnd,is,js,iibnd,jjbnd,ifs,nkk
  integer                  :: switch
...
!  sigma_e=20.d0*d_e
  sigma_e=40.d0*d_e
  !
  allocate(imselfen_w(epts))
  !
  ! We choose how many state-points we are examining
  !
  nkk=101
  !
  do ik=35,35
     !
     ! We initialize interesting variables
     !
     imselfen_w(:)=0.d0
     !
     ! momentum steps; it defines the direction i will follow
     !
     dkvec(1)=0.00d0
     dkvec(2)=0.001d0
     dkvec(3)=0.000d0
     !
     ! momentum we are examining in each step
     !
     kpoint(1)=0.666d0+dkvec(1)*(ik-31)
     kpoint(2)=0.666d0+dkvec(2)*(ik-31)
     kpoint(3)=0.d0+dkvec(3)*(ik-31)
     !
     ! the band we choose -> (k,n) state defined
     !
     jbnd=4
     !
     ...
     !
     ! summation on q (phonon) momentum
     !
     do iq=1,nqfmesh
        !
        ...
        !
        ! summation on final state (k+q) band
        !
!        do ibnd=1,num_wann
        do ibnd=4,5
           !
           ...
           !
           ! if final state not present in the "energy-window" we are working on, cycle
           !
           if ((e_min-w_max-4*sigma_e).gt.(ekq-ef).or.(e_max+w_max+4*sigma_e).lt.(ekq-ef)) cycle
           !
           ! Definition of g(k,q,m,n,:)
           !
           g_me(:)=g_elph(ibnd,jbnd,:)
           !
           ! Summation on different modes
           !
           do imode=1,3*nat
              !
              ! phonon mode -> (q,nu)
...
!              g2=weight_ph(wqv)*(abs(g_me(imode))**2.d0)/(2.d0*wqv)
              ...
              if (wqv.lt.0.001) g2=0.0d0
              !
              !$omp parallel default(none) &
              !$omp shared(epts,e_min,d_e,sigma_e,ekq,wqv,ekk,trshld,imselfen_w,g2,pi,ef) &
              !$omp private(ipts,e) 
              !
              !$omp do
              !
              do ipts=1,epts
                 ...
                 e=e_min+d_e*(ipts-1)
                 !
                 if (e.gt.(ekq-ef).and.ekq.gt.ef) then
                    !
                    imselfen_w(ipts)=imselfen_w(ipts)+g2*pi*exp(-((e-wqv-(ekq-ef))/sigma_e)**2.d0)/(sigma_e*sqrt(pi))
                    !
                 elseif (e.lt.(ekq-ef).and.ekq.lt.ef) then
                    !
                    imselfen_w(ipts)=imselfen_w(ipts)+g2*pi*exp(-((e+wqv-(ekq-ef))/sigma_e)**2.d0)/(sigma_e*sqrt(pi))
                    !
                 endif
     ...
     enddo !iq
     !
     ! We normalize with respect our Brillouin Zone mesh
     !
     imselfen_w(:)=imselfen_w(:)/nqfmesh
     !
     ! Write imaginary part for each (k,n) state
     !
     ...
     !
     if (ik<10) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('im_selfen_')//trim(ik_loc1)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('im_selfen_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
     elseif (ik<100) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('im_selfen_')//trim(ik_loc2)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('im_selfen_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
     elseif (ik<1000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('im_selfen_')//trim(ik_loc3)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('im_selfen_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
     elseif (ik<10000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('im_selfen_')//trim(ik_loc4)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('im_selfen_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
     endif
     !
     do ipts=1,epts
        !
        write(10*ik+1,'(100f16.8)') e_min+d_e*(ipts-1),imselfen_w(ipts)
        write(10*ik+100000+1) imselfen_w(ipts)
        !
     enddo !ipts
     !
     close(10*ik+1)
