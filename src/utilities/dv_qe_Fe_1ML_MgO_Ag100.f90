program dv_hand
  ...
  use intw_ph, only: frc, q_irr_cryst, QE_folder_sym_q, sym_G_q, symlink_q, &
                     ...
                     mat_inv_four_t, dvq_local, get_dv, q_irr!, dvscf_cart
  ...
  real(kind=dp), dimension(3) :: r, q
  integer :: record_lengh, ios
...
!! END_DEBUG
  ...
  !
  ph_dir='./'
  fc_mat='mgo-ag100_441.fc'
  data_dir='data_dir/'
  data_name='data-file.'
  dvscf_dir='dvscf_dir/'
  dvscf_name='mgo-ag100.dvscf_q'
  qlist='qirred441.dat'
  nq1   = 4
  nq2   = 4
  nq3   = 1
...
!! END_DEBUG
  ...
  do iq=1,nqirr
    ...
    print'(i4,3f16.8)', iq, q_irr_cryst(:,iq)
  enddo
  !
  !================================================================================
      ...
      do ir3=1,nr3
        do ir2=1,nr2
          do ir1=1,nr1
            ...
            dvq_local(ir, 1,1,1) = dvq_local(ir, 1,1,1) * exp(cmplx_i*qr)
            !
          enddo
        enddo
      enddo
    ...
    do irr2=1,nq2
      do irr3=1,nq3
        ...
        !   m=1.0_dp/(ndegen(iq)*nqmesh)
          ...
          ! Take into acount the translation of the supercell to be centered ( q is in the range [-2,2) )
          qrr = tpi*(qpoint(1)*(irr1-1-2) + qpoint(2)*(irr2-1-2) + qpoint(3)*(irr3-1))
          !
          dvq_local=cmplx_0
          call get_dv(iq,qpoint,3*nat,dvq_local)
          !
          do ir3=1,nr3_sc
            do ir2=1,nr2_sc
              do ir1=1,nr1_sc
                ...
                ! Calculate the phase
                do i=1, 3
                  r(i) = real(ir1-1,dp)/nr1*at(i,1) + real(ir2-1,dp)/nr2*at(i,2) + real(ir3-1,dp)/nr3*at(i,3)
                enddo
                do i=1, 3
                  q(i) = qpoint(1)*bg(i,1) + qpoint(2)*bg(i,2) + qpoint(3)*bg(i,3)
                enddo
                !
                !qr=tpi*(qpoint(1)*real(ir1-1,dp)/nr1 + qpoint(2)*real(ir2-1,dp)/nr2 + qpoint(3)*real(ir3-1,dp)/nr3)
                qr=tpi*( q(1)*r(1) + q(2)*r(2) + q(3)*r(3) )
                !
                do imode=1,3*nat
                  !
                  ia = (imode-1)/3 + 1
                  ...
                  dv_sc(ir1,ir2,ir3, imode_sc) = &
                        dv_sc(ir1,ir2,ir3, imode_sc) + &
                        m* ( dvq_local(ir, imode,1,1) * exp(cmplx_i*qr)* exp(-cmplx_i*qrr) )! + &
...
!            if ( k==nint( tau(3,ia)/at(3,3)*nr3_sc + 1 ) ) then
...
!write(1002,*) dv_sc(:,:,:, 2)
  ...
  !
  inquire(iolength=record_lengh) dv_sc(:,:,:,1)
  !
  open (unit=1243, file ='mgo-ag100_R441.dvscf_q1',iostat = ios, &
            form = 'unformatted', status = 'new', access = 'direct', recl=record_lengh )
  !
  do imode=1,3*nat*nq1*nq2*nq3
   write(unit=1243, rec=imode,  iostat = ios) dv_sc(:,:,:,imode)
  enddo
  !
  close(1243)
  !
