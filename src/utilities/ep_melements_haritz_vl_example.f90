program ep_melements
  ...
  ! Variables used to calculate the matrix element in real space
  complex(kind=dp), allocatable :: wfc_k_r(:)  ! wave function k+q in G space (nG_max)
  complex(kind=dp), allocatable :: wfc_kq_r(:)  ! wave function k+q in G space (nG_max)
  complex(kind=dp), allocatable :: local(:)
  integer :: ir

  logical :: as_intw=.false. ! the way used to calculate the matrix elements
  ...
  allocate( all_wfc_k(nG_max,nbands), wfc_kq(nG_max) )
  list_iG  = 0
  eig      = ZERO
  all_wfc_k  = cmplx_0
  wfc_kq = cmplx_0
  ...
  ! vkqb = vkb
  !
  allocate( ep_mat_el(nbands_selected,3*nat) )
  ep_mat_el=cmplx_0
  !
  ...
  if (calc_epmat) then
    ...
    call cpu_time(t0)
    !
    if (as_intw) then ! One way to calculate the matrix elements (as in intw)
      !
      ...
      do jband=1,num_bands
        ...
        do imode=1,3*nat
          !
          call matrix_element(wfc_kq,dvpsi(:,jband,1,1,imode),ep_mat_el(MPI_nbands_initial+jband-1,imode))
          !
        enddo
        !
      enddo
      !
    else ! Another way to calculate the matrix elements (calculateing everything in real space)
      !
      ! allocate some variables
      allocate( wfc_k_r(nr1*nr2*nr3) )
      allocate( wfc_kq_r(nr1*nr2*nr3) )
      allocate(local(nr1*nr2*nr3))
      !
      ! Fourier transform the initial wave function to r space
      call wfc_from_g_to_r_haritz( list_iG, wfc_kq, wfc_kq_r )

      do imode=1,3*nat
        !
        ! Calculate  dV_loc
        local = cmplx_0
        call calculate_local_part_dv( (/0.0_dp,0.0_dp,0.0_dp/), imode, local )
        !
        do jband=1,num_bands
          !
          ! Fourier transform the final wave function to r space
          call wfc_from_g_to_r_haritz( list_iG, wfc_k(:,jband,1), wfc_k_r )
          !
          ! Calculate the matrix element < psi_kq | dV_nl | psi_k >
          ep_mat_el(MPI_nbands_initial+jband-1,imode)=cmplx_0
          do ir=1,nr1*nr2*nr3
            !
            ep_mat_el(MPI_nbands_initial+jband-1,imode) = ep_mat_el(MPI_nbands_initial+jband-1,imode) + &
                                                      conjg(wfc_kq_r(ir))*local(ir)*(wfc_k_r(ir)) / &
                                                      (nr1*nr2*nr3)
            !
          enddo
          !
    ...
    if (myNode==master) then
      ...
      do imode=1,3*nat
        do jband=1,nbands_selected
          print"(a6,3i5,4x,2f15.10,4x,f15.10)", "haritz", iband, nbands_initial+jband-1, imode, ep_mat_el(jband,imode), abs(ep_mat_el(jband,imode))
        enddo
      enddo
      !
      !
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine read_input_file()
    ...
    NAMELIST / input_ep_melements / mesh_dir, prefix, iband, ispin, as_intw
    !
    ! Set default values
    mesh_dir = './'
    prefix = 'g'
    ...
    call MPI_Bcast( as_intw, 1, MPI_logical, master, MPI_COMM_WORLD, MPIerror )
    !
