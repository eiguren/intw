program eliashberg_full_no_spin
  ...
  real(dp) :: trshld,trshld1,trshld2,w_max,dkvec(3)
  real(dp) :: ekq,ekk,wqv,wqv_au
  ...
  !local/aux variables
  real(dp)                 :: sigma_e,qmoduloa
  integer                  :: nbands_loc,irk,irq
  integer                  :: i,j,k
  integer                  :: ig,ibnd,jbnd,is,js,nkk
  integer                  :: switch
  logical                  :: read_status,have_nnkp
  character(256)           :: nnkp_file,method
  character(len=4)         :: iq_loc
  real(dp) :: rytoev=13.605698066d0 , amutory=911.44424213227238468637d0
  ...
  if (kqb_2_ReRpw_me) then
     ...
     if (kqb_2_kqw_me) then
        ...
        do iq=1,nqmesh
           ...
           open(unit=ep_unit,&
           file=trim(trim(mesh_dir)//trim('EP_DATA_LEIOA/')//trim(ep_mat_file)//trim('_')//adjustl(iq_loc)),&
...
!     trshld=1.9594 !C graph
  ...
  open(unit=1001,file=trim(trim(mesh_dir)//trim('lambda.dat')))
  !
  !==============================================================================================
  ! Calculation of the state dependent EPC strength/mass enhancement parameter lambda for the
  ...
  ! We know:
  !
  ! lambda(k,n)=Sum_q_m_nu[|g(q,k,m,n,nu)|^2*Delta[E(k,n)-E(k+q,m)+/-wqv]/wqv]
  !
...
!        dkvec(3)=0.000d0
!        !
!        kpoint(1)=0.667d0+dkvec(1)*(ik-1)
!        kpoint(2)=0.667d0-0.125d0+dkvec(2)*(ik-1)
...
!     else
        ...
        dkvec(3)=0.000d0
        !
        kpoint(1)=0.333d0+dkvec(1)*(ik-1)
        kpoint(2)=0.334d0+dkvec(2)*(ik-1)
        kpoint(3)=0.000d0+dkvec(3)*(ik-126)
!        kpoint(1)=0.667d0+dkvec(1)*(ik-126)
!        kpoint(2)=0.667d0+dkvec(2)*(ik-126)
!        kpoint(3)=0.000d0+dkvec(3)*(ik-126)
        !
!        jbnd=1
!        dkvec(1)=0.0002d0
...
!     endif
     ...
     write(1314,'(i10,3f8.5,f12.6,100f15.8)') ik,kpoint(:), &
           sqrt(kpoint_cart(1)**2+kpoint_cart(2)**2+kpoint_cart(3)**2),eigen(4,ikpt_k)+trshld,eigen(5,ikpt_k)+trshld
!     if (ik.lt.501) then
!        !
!        write(1314,'(i10,3f8.5,f12.6,100f15.8)') ik,kpoint(:), &
...
!        do ibnd=1,num_wann
        do ibnd=4,5
           ...
           !
           if (abs(ekk-ekq).gt.w_max+4*sigma_e) then
              !
           else
              !
              iq_cont=iq_cont+1
     ...
     !
     !$omp parallel default(none) &
     !$omp shared(nqfmesh_2,ik,nkk,list_iq,qfmesh,kpoint,nkf1,nkf2,nkf3,eigen,iwant,U_ph,omega,nrr_q,irr_q,ndegen_q,g_Rp,U_el,rytoev,nat,eps_6,lambda) &
     !$omp shared(ekk,sigma_e,pi,nqfmesh,bg,cmplx_0,ikpt_k,amutory,jbnd) &
     !$omp private(iq_2,iq,ibnd,qpoint,qpoint_cart,i,j,k,kqpoint_in_1bz,GKQ_bz,ikpt_kq,ekq,U_phon,omega_phon,g_wannier,g_elph,g_me,imode,wqv,wqv_au) &
     !$omp private(g2)
     !
     !$omp do
     !
     do iq_2=1,nqfmesh_2
        ...
        !
        ! Filter q->0 |q|<0.01, omegas are not well diagonalized for these points,
        ! we quit them from the summation
        !
!        qmoduloa=sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2)
!        if (qmoduloa.lt.0.01) cycle
!        qmoduloa=sqrt((qpoint_cart(1)-1.d0)**2+(qpoint_cart(2)-1.d0/sqrt(3.d0))**2+(qpoint_cart(3)-0.d0)**2)
!        if (qmoduloa.lt.0.01) cycle
!        qmoduloa=sqrt((qpoint_cart(1)-0.d0)**2+(qpoint_cart(2)-2.d0/sqrt(3.d0))**2+(qpoint_cart(3)-0.d0)**2)
!        if (qmoduloa.lt.0.01) cycle
!        qmoduloa=sqrt((qpoint_cart(1)-1.d0)**2+(qpoint_cart(2)-sqrt(3.d0))**2+(qpoint_cart(3)-0.d0)**2)
!        if (qmoduloa.lt.0.01) cycle
        !
        call find_k_1BZ_and_G(kpoint+qpoint,nkf1,nkf2,nkf3,i,j,k,kqpoint_in_1bz,GKQ_bz)
        call switch_indices(nkf1,nkf2,nkf3,ikpt_kq,i,j,k,+1)
        ...
        call G_kqw2kqb_no_spin(U_el(:,:,ikpt_kq),U_el(:,:,ikpt_k),U_phon(:,:)/sqrt(amutory),g_wannier,g_elph)
        !
...
!                    .or.imode.eq.28.or.imode.eq.30.or.imode.eq.34.or.imode.eq.35.or.imode.eq.36) cycle
           ...
           if (wqv.lt.eps_6) then
              ...
              !
              g2=weight_ph(wqv)*(abs(g_me(imode))**2.d0)/(2.d0*wqv_au)
!              g2=abs(g_me(imode))**2.d0/(2.d0*wqv_au)
              !
           endif !wqv > 0
           !
           if (abs(ekk-ekq+wqv).gt.4*sigma_e) then
              !
           else
              !
              lambda(ik)=lambda(ik)+(g2/wqv)*(exp(-((ekk-ekq+wqv)/sigma_e)**2.d0)/(sigma_e*sqrt(pi)))
              !
           endif
              !
           if (abs(ekk-ekq-wqv).gt.4*sigma_e) then
     ...
     !$omp end parallel
     !
     ! We normalized the lambda parameter
     !
     lambda(ik)=lambda(ik)/nqfmesh
     !
     ! We write our results
     !
     write(1001,'(i5,100f16.8)') ik,lambda(ik)
     !
  enddo !ik
  !
  close(1001)
