program lambda_full_nk
  ...
  integer :: epts,jpts
  real(dp) :: ef,e_min,e_max,d_e,sigma_e
  ...
  do ik=1,nkk
     ...
     !
     if (ik<10) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
     elseif (ik<100) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
     elseif (ik<1000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
     elseif (ik<10000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
     endif
     !
     do ipts=1,wpts
        !
     ...
     imselfen_w(:)=0.d0
     !
     call imselfen_calculate_full_t0(wpts,w_min,w_max,d_w,epts,e_min,e_max,d_e,eliash_fplus,eliash_fminus,imselfen_w,ef)
     call reselfen_calculate_full_t0(wpts,w_min,w_max,d_w,epts,e_min,e_max,d_e,eliash_fplus,eliash_fminus,reselfen_w,ef)
     !
     do ipts=1,epts
        !
        write(1314,'(100f16.8)') e_min+d_e*(ipts-1),reselfen_w(ipts),imselfen_w(ipts)
        !
     enddo !ipts
     !
  enddo !nkk
