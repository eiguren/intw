...
program ep_on_trFS_dV
  ...
  write(*,*)' use_exclude_bands =', trim(use_exclude_bands)
  ...
  if ( trim(ep_interp_bands) == 'intw_bands' ) then
    nfs_sheets_tot = num_bands_intw
    allocate ( nfs_sheet(nfs_sheets_tot) )
    do ib = 1, num_bands_intw
      nfs_sheet(ib) = ib
    end do
  else if ( trim(ep_interp_bands) == 'ef_crossing' ) then
  ...
  file_scf=trim(mesh_dir)//trim(prefix)//'.scf.in'
  ...
  if ( .not. have_nscf) then
    ...
    ! creat directory for nscf calculation prefix-nscf.save
  ...
  ! Wigner-Seitz R points (same as when we interpolate phonon bands)
