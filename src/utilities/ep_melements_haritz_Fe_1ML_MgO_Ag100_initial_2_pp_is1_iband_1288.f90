program ep_melements
  ...
  mesh_dir = '/scratch/hgarai/Kalkuluak/Fe-MgO-Ag100/1ML/M_ELEMENTS/'
!  mesh_dir = '/home/haritz/Kalkuluak/Ag/Fe-MgO-Ag100/1ML/ph/Fe-MgO-Ag100/spin/smearing_0.01/'
  ...
  ep_mat_file=trim("ep_mat_is_1_iband_1288_PP.dat")
  nq1=1
  nq2=1
  nq3=1
  nqirr=1
  ...
  if (myNode==master) call get_e_fermi()
  ...
  if (calc_epmat) then
    ...
    if (myNode==master) then
      ...
      open( unit=ep_unit, file=trim(trim(mesh_dir)//trim(ep_mat_file)//trim("_2")), iostat=ierr, &
