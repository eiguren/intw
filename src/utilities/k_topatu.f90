program k_topatu
  ...
  !Wigner-Seitz
  integer :: nrr_k, nrr_q, ire
  real(dp),allocatable :: wslen(:)
  ...
  real(dp) :: k_K(3),k_V(3),k_step(3)

  !matrix-elements
  complex(dp),allocatable :: g_Req(:,:,:,:,:,:,:), g_ReRp(:,:,:,:,:,:,:)
  complex(dp),allocatable :: ag_ReRp(:,:,:,:,:,:), g_kq(:,:,:,:,:,:,:)
  complex(dp),allocatable :: g_Rp(:,:,:,:,:,:), g_smooth(:,:,:,:,:)
  complex(dp),allocatable :: g_cart(:,:,:,:,:), g_elph(:,:,:,:,:)
  real(dp) :: g2

  !Bloch2Wannier
  logical :: bloch2wan_me,bloch2wan_el,bloch2wan_ph,smoothbloch_me
  real(dp) :: arg, xen
  complex(dp) :: fac
  integer :: g_wann_unit

  !selfenergy
  complex(dp),allocatable :: el_selfen(:,:,:)
  real(dp) :: temp  ! temperature of the system
  real(dp) :: eps  ! energy of the incoming electron
  real(dp) :: delta  ! broadening width of the lorentzian
  real(dp) :: efermi,trshld,step
  integer :: nstep,istep
  real(dp),allocatable :: re_selfen_q(:,:,:,:),im_selfen_q(:,:,:,:)
  real(dp),allocatable :: re_selfen(:,:,:),im_selfen(:,:,:)
  real(dp) :: term_re,term_im,wq,ekq,fe,nw

  !Hamiltonian
  complex(dp),allocatable :: ham_coarse(:,:,:)
  complex(dp),allocatable :: ham_wann(:,:,:)
  complex(dp),allocatable :: a_h_wann(:,:)
  real(dp),allocatable :: eigen(:,:)
  complex(dp),allocatable :: U_el(:,:,:)
  integer :: h_wann_unit
  logical,allocatable :: ham_passed(:)

  !Dynamical Matrix
  complex(dp),allocatable :: dyn_coarse(:,:,:)
  complex(dp),allocatable :: dyn_wann(:,:,:)
  complex(dp),allocatable :: a_ph_wann(:,:)
  real(dp),allocatable :: omega(:,:)
  complex(dp),allocatable :: U_ph(:,:,:)
  integer :: ph_wann_unit
  logical,allocatable :: dyn_passed(:)
  ...
  integer                  :: ii, jj, kk
  integer                  :: jr, kr
  integer                  :: ig, ibnd, jbnd, ipol, jpol, iibnd, jjbnd
  integer                  :: switch
...
!  call  read_allq_dvr(nqirr, 3*nat)
  ...
  !================================================================================
  allocate(wslen(20*nk1*nk2*nk3),ndegen_k(20*nk1*nk2*nk3),ndegen_q(20*nq1*nq2*nq3))
  if (allocated(irvec)) deallocate(irvec)
  allocate(irvec(3,20*nk1*nk2*nk3))
  !
  ...
  write(*,*)' '
  !
  allocate(ham_wann(num_wann,num_wann,nrr_k))
  allocate(a_h_wann(num_wann,num_wann))
  ham_wann(:,:,:)=cmplx_0
  !
  if (bloch2wan_el) then
     write(*,*)'We must calculate the Hamiltonians in the Wannier Representation'
     !
     call H_bloch2wannier(nrr_k,kmesh,ham_wann,wslen)
     !
     write(*,*)' '
     !
     !===============================================================================
     ! Writing the Hamiltoninas in the Wannier Representation
     !===============================================================================    
     h_wann_unit=find_free_unit()
     inquire(iolength=record_lengh) a_h_wann
     open(unit=h_wann_unit,file=trim(trim(mesh_dir)//trim(h_wann_file)),iostat=ierr,&
     form='unformatted',status='unknown',access='direct',recl=record_lengh)
     !
     do ir=1,nrr_k
        write(*,*)'Writing Wannier Hamiltonians ir = ',ir,'/',nrr_k
        write(unit=h_wann_unit,rec=ir,iostat=ierr) ham_wann(:,:,ir)
     enddo
     !
     close(unit=h_wann_unit)
  else
  !
  ! If we have already calculated the Hamiltonians in Wannier representation, we read them
  !
  !=================================================================================
  ! Reading the Hamiltonians in Wannier Representation
  !=================================================================================
     h_wann_unit=find_free_unit()
     inquire(iolength=record_lengh) a_h_wann
     open(unit=h_wann_unit,file=trim(trim(mesh_dir)//trim(h_wann_file)),iostat=ierr,&
     ...
     !
     do ir=1,nrr_k
        write(*,*)'Reading Wannier Hamiltonians ir = ',ir,'/',nrr_k
        read(unit=h_wann_unit,rec=ir,iostat=ierr) ham_wann(:,:,ir)
     enddo
     !
     close(unit=h_wann_unit)
  endif
...
!  k_V(2)=0.0d0
  ...
  !
  allocate(kfmesh(3,nstep))
  !
  allocate(U_el(num_wann,num_wann,nstep))
  allocate(eigen(num_wann,nstep))
  U_el(:,:,:)=cmplx_0
  eigen(:,:)=0.0d0
  !
  do ik=1,nstep
     !
     kpoint(:)=k_V(:)+k_step(:)*(ik-1)
     kfmesh(:,ik)=kpoint(:)
     !
     call H_wannier2bloch(nrr_k,kpoint,ndegen_k,ham_wann,U_el(:,:,ik),eigen(:,ik))
     eigen(:,ik)=eigen(:,ik)+trshld
     !
     write(778,'(f12.6,3f6.3)') eigen(3,ik),kfmesh(:,ik)
     !
  enddo
  !
!  eps=0.0001
...
!     do ik=1,nstep
!        !
!        if (eigen(2,ik).gt.(xen-eps).and.eigen(2,ik).lt.(xen+eps)) then
!           !
!           write(778,'(f6.3)') xen
!           write(778,'(10f6.3)') kfmesh(:,ik)
!           exit
!           !
!        endif
!        !
