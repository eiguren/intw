...
program ep_on_trFS_wannier
    ...
    use intw_input_parameters, only: nk1, nk2, nk3, mesh_dir, prefix, read_input, &
            nq1, nq2, nq3, ph_dir, fc_mat, chemical_potential
...
!   (doy las dimensiones a mano hasta tener el test)
!================================================================================
...
!================================================================================
    ...
    allocate (gmatL_wann(3*nat,num_wann_intw,num_wann_intw,nrpts_q,nrpts) )
    ...
    do iat=1,3*nat
        ...
        do irq=1,nrpts_q
           ...
           do ir=1,nrpts
              gmatL_wann(iat,:,:,irq,ir) = gmatL_wann(iat,:,:,irq,ir) + gmat_aux(:,:,iq,ir) * facq
           end do
        end do
        end do
...
!================================================================================
 ...
 !================================================================================
 ! TESTS given iq=3 of nqmesh, interpolate at all ik in nkmesh,
...
goto 666 ! skip tests
 ...
 open (17, file='cu_band.dat', status='old')
   read(16,*) i
   do ik=1,i
      read(16,*) kpoint
      read(17,*) ener
      call interpolate_1k (kpoint, eig_kint, u_kint)
      write(998,'(10e16.4)') ener, eig_kint
   end do
 close(16)
 close(17)
 ...
 ! Check decay
 do irq=1,nrpts_q
   rcart = real(irvec_q(:,irq), dp)
   call cryst_to_cart (1, rcart, at, 1)
   write(998,'(10e16.4)') alat*sqrt(sum(rcart*rcart)), (abs(gmatL_wann(iat,6,6,irq,1)), iat=1,3*nat)
 end do
 write(998,*) '  '
 write(998,*) '  '
 do ir=1,nrpts
   rcart = real(irvec(:,ir), dp)
   call cryst_to_cart (1, rcart, at, 1)
   write(998,'(10e16.4)') alat*sqrt(sum(rcart*rcart)), (abs(gmatL_wann(iat,6,6,1,ir) ), iat=1,3*nat)
 end do
 write(998,*) '  '
 write(998,*) '  '

! Test: retrieve elements for one of the coarse k+q,k points
 iq=3
 is=1
 js=1
 qpoint = qmesh(:,iq)
 do ik=8,8 !1,nkmesh
    ...
    do iat=1,3*nat
         ...
         !       abs(gmat_int_rot(ibp,ib)), &
         !      ep_mat_el_coarse(iq,ik,ibp,ib,js,is,iat), abs(ep_mat_el_coarse(iq,ik,ibp,ib,js,is,iat) )
         !end do

         print *, 'allbands with umesh again'
         do ib=1,num_bands_intw
           ibp=ib
           cdum = cmplx_0
           do iw=1,num_wann_intw
           do jw=1,num_wann_intw
              cdum = cdum + u_mesh(ibp,iw,ikp) * gmat_int(iw,jw) * conjg(u_mesh(ib,jw,ik) )
           end do
           end do
           write(999,fmt="(5i6,6e16.6)") ikp, ik, ibp, ib, iat, cdum, abs(cdum),&
               ep_mat_el_coarse(iq,ik,ibp,ib,js,is,iat), abs(ep_mat_el_coarse(iq,ik,ibp,ib,js,is,iat) )
         end do
 ...
 kpoint = kmesh(:,ik)
 call interpolate_1k (kpoint, eig_kint, u_kint)
 gmat_int_rot =  matmul(u_mesh(1:num_wann_intw,:,ik),transpose(conjg(u_mesh(1:num_wann_intw,:,ik))) )
 do iw=1,num_wann_intw
   write(*,'(20e16.4)') gmat_int_rot(iw,:)
 end do

 stop

666 continue  !tests skipped
...
! (aep_mat_el elements are stored only for the needed pair of sheets for a given kk')
  ...
  do iksp = 1, nkpt_tr(ishp)
      ...
      ! TODO some of these can be done out of the 3nat loop to go faster
      do iat=1,3*nat
         gmat_aux1 = cmplx_0
         gmat_int = cmplx_0
         gmat_int_rot = cmplx_0
         ...
         call wann_FT_1index_1k (kpoint, gmat_aux1(:,:,:), gmat_int(:,:))
         gmat_int_rot(:,:) = matmul(u_kqint, matmul(gmat_int, u_kint))   ! TODO spins
