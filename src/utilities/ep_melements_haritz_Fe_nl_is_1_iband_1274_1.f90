program ep_melements
  ...
  !
  use intw_haritz, only: stdin, stdout
  !
  ...
  use intw_haritz, only: gamma_only, get_gamma_only_haritz, get_spin_haritz, &
                         nlm, generate_nl_haritz, wfc_from_g_to_r_haritz, &
                         get_e_fermi_haritz
  !
    ...
    if (myNode==master) then
      ...
      do imode=1,3*nat
        ...
        do jband=1,nbands_selected
          print"(3i5,5x,2f18.12)", iband, nbands_initial+jband-1, imode, ep_mat_el(jband,imode)
        enddo
        !
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine matrix_element(bra_wfc, dv_ket_wfc, ep)
    ...
    if (gamma_only) then
      do ig=1,nG_max
        ep = ep + 2.0_dp*real( conjg(wfc_kq(ig))*dv_ket_wfc(ig) )
      enddo
      ep = ep - conjg(wfc_kq(1))*dv_ket_wfc(1)
    else
      do ig=1,nG_max
        ep = ep + conjg(wfc_kq(ig))*dv_ket_wfc(ig)
      enddo
    endif
    !
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine read_input_file()
    ...
    NAMELIST / input_ep_melements / mesh_dir, prefix, iband, ispin
    !
    ! Set default values
    ...
    ep_mat_file=trim("ep_mat_is_1_iband_1274_jband_1279-1328_NL.dat")
    nq1=1
    nq2=1
    nq3=1
    nqirr=1
