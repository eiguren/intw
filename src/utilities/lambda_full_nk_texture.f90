program lambda_full_nk_texture
  ...
  complex(dp), allocatable :: aep_mat_el(:,:,:,:,:,:,:), ep_mat_el(:,:,:,:,:,:), aep_mat_el_2(:,:,:,:,:)
  ...
  integer                  :: ii,jj,kk
  integer                  :: jr,kr
  integer                  :: ig,ibnd,jbnd,ipol,jpol,iibnd,jjbnd,ifs,nkk,nfs,is,js
  integer                  :: switch
...
!     do ibnd=jbnd,jbnd
        ...
        !
!        if (abs(ekk-ekq).gt.w_max*10.d0) then
!        if (abs(ekk-ekq).gt.w_max) then
        if (abs(ekk-ekq).gt.w_max*3.d0) then
           !
        else
           !
           if (iwant) then
...
!           call add_q_m_nu_term_full_no_spin(w_min,w_max,wpts,sigma_w,sigma_e,ekk,ekq,omega_phon,g_me,eliash_fplus,eliash_fminus,3)
           !
        endif
        !
     enddo !ibnd
     ...
     !
     call lambda_calculate_full_t0(wpts,w_min,w_max,d_w,eliash_fplus,eliash_fminus,lambda(ikpt_kq))
     !
  enddo !iq
  !
  write(*,*) 'lambda = ',sum(lambda(:))
  !
  allocate(lambda_tex(-nkf1:nkf1,-nkf2:nkf2))
  lambda_tex=0.0d0
  !
  do ik=1,nkfmesh
     !
     ...
     call find_k_1BZ_and_G(kpoint,nkf1,nkf2,nkf3,i,j,k,kpoint_in_1bz,GKQ_bz)
     !
     lambda_tex((i-1)-nkf1,(j-1)-nkf2)=lambda(ik)
     lambda_tex((i-1)-nkf1,j-1)=lambda(ik)
     lambda_tex(i-1,(j-1)-nkf2)=lambda(ik)
     lambda_tex(i-1,j-1)=lambda(ik)
     !
  enddo !ik
  !
  do ik=-nkf1,nkf1-1
     !
     lambda_tex(ik,nkf2)=lambda_tex(ik,0)
     !
  enddo !ik
  !
  do ik=-nkf2,nkf2-1
     !
     lambda_tex(nkf1,ik)=lambda_tex(0,ik)
     !
  enddo !ik
  !
  lambda_tex(nkf1,nkf2)=lambda_tex(0,0)
  !
  open(unit=11,file=trim(trim(mesh_dir)//trim('lambda_tex.dat')))
  !
  do ikx=-nkf1,nkf1
     do iky=-nkf2,nkf2
        !
        ...
        !
        write(11,'(2f16.8,f20.15)') kpoint(1),kpoint(2),lambda_tex(ikx,iky)
        !
     enddo !iky
  enddo !ikx
  !
...
end program lambda_full_nk_texture
