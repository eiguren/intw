program imaginary_selfen
  ...
  allocate (aW_rot_ep_mat_el(nqmesh,nk1*nk2*nk3,num_wann,num_wann,3*nat))
  ...
  allocate (aW_rot_ep_mat_el_ext(nqmesh,nk1*nk2*nk3,num_wann,num_wann,3*nat))
  allocate (W_rot_ep_mat_el(nk1*nk2*nk3,num_wann,num_wann,3*nat))
  ...
  kqb_2_ReRpw_me_ext=.false.
  kqb_2_kqw_me=.false.
  kqb_2_kqw_me_ext=.false.
  h_kb_2_Rew=.false.
  dm_q_2_Rp=.false.
  !
  !================================================================================
  ...
  !
  allocate(g_ReRp(nrr_q,nrr_k,num_wann,num_wann,3*nat))
  allocate(g_ReRp_ext(nrr_q,nrr_k,num_wann,num_wann,3*nat))
  g_ReRp=cmplx_0
  g_ReRp_ext=cmplx_0
  !
  allocate(ag_ReRp(nrr_k,num_wann,num_wann,3*nat)) ! For writing
  !
  !===================================================================================================
  ! Reciprocal Bloch coarse to Real Wannier (Wannier Rotation + Fourier Transform): Matrix Elements KS
  !===================================================================================================
  if (kqb_2_ReRpw_me) then
        ...
        do iq=1,nqmesh
           ...
           do ik=1,nkmesh
              ...
              do imode=1,3*nat
                 ...
                 !
                 call Wrotate_matrix_elements_no_spin(ikpt_kq,ikpt_k,u_mesh,&
                 aep_mat_el_2(iq,ikpt_k,1:nbands_loc,1:nbands_loc,imode),&
                 aW_rot_ep_mat_el(iq,ikpt_k,1:num_wann,1:num_wann,imode))
                 !
              enddo !imode
           enddo !ik
           !
     ...
     !
     deallocate(aep_mat_el,aW_rot_ep_mat_el)
     !     
     !==============================================================================================
     ...
     ! 2) g(Re,q) -> g(Re,Rp)
     !
     allocate(g_Req(nqmesh,nrr_k,num_wann,num_wann,3*nat))
     g_Req=cmplx_0
     !
     write(*,*)'------------------------------------------------------------------'
     write(*,*)'------------------------------------------------------------------'
     ...
     !
     call G_kqw2Reqw_no_spin(nrr_k,irr_k,kmesh,g_kq,g_Req)
     !
     deallocate(g_kq)
     !
     write(*,*)'------------------------------------------------------------------'
     ...
     !
     call G_Reqw2ReRpw_no_spin(nrr_q,nrr_k,irr_q,irr_k,qmesh,g_Req,g_ReRp)
     !
     deallocate(g_Req)
     !
     write(*,*)'------------------------------------------------------------------'
  ...
  !
  !===================================================================================================
  ! Reciprocal Bloch coarse to Real Wannier (Wannier Rotation + Fourier Transform): Matrix Elements EXT
  !===================================================================================================
  if (kqb_2_ReRpw_me_ext) then
     !
     ! If we have not yet calculated the matrix elements in the Reciprocal Wannier Representation, we do it
     !
     !=============================================================================================
     ...
     if (kqb_2_kqw_me_ext) then
        ...
        do iq=1,nqmesh
           ...
           inquire(iolength=record_lengh) W_rot_ep_mat_el
           open (unit=Wep_unit,&
           file=trim(trim(mesh_dir)//trim(ep_mat_file)//trim('W_')//adjustl(iq_loc)),&
           iostat=ierr,form='unformatted',status='unknown',access='direct',recl=record_lengh)
           !
           write(*,*)'Calculating Reciprocal Wannier matrix elements in the coarse mesh for iq = ',iq,'/',nqmesh,ierr
           !
           do ik=1,nkmesh
              ...
              do imode=1,3*nat
                 ...
                 !
                 call Wrotate_matrix_elements_no_spin(ikpt_kq,ikpt_k,u_mesh,&
                 aep_mat_el_2_ext(iq,ikpt_k,1:nbands_loc,1:nbands_loc,imode),&
                 aW_rot_ep_mat_el_ext(iq,ikpt_k,1:num_wann,1:num_wann,imode))
                 !
              enddo !imode
           enddo !ik
           !
           write(*,*)'Writing Reciprocal Wannier matrix elements in the coarse mesh for iq = ',iq,'/',nqmesh
           write(unit=Wep_unit,rec=1,iostat=ierr) aW_rot_ep_mat_el_ext(iq,:,:,:,:)
           close(unit=Wep_unit)
           !
           write(*,*)' '
        enddo !iq
        ...
        do iq=1,nqmesh
           ...
           inquire(iolength=record_lengh) W_rot_ep_mat_el
           open(unit=Wep_unit,&
           file=trim(trim(mesh_dir)//trim('EPW_DATA_EXT/')//trim(ep_mat_file)//trim('W_')//adjustl(iq_loc)),&
           iostat=ierr,form='unformatted',status='unknown',access='direct',recl=record_lengh)
           !
           write(*,*)'Reading Reciprocal Wannier matrix elements in the coarse mesh for iq = ',iq,'/',nqmesh
           read(unit=Wep_unit,rec=1,iostat=ierr)  aW_rot_ep_mat_el_ext(iq,:,:,:,:)
           close(unit=Wep_unit)
           !
           write(*,*)' '
        enddo
     ...
     ! We change of name variable, for comodity
     !
     g_kq_ext(:,:,:,:,:)=aW_rot_ep_mat_el_ext(:,:,:,:,:)
     !
     deallocate(aep_mat_el_ext,ep_mat_el,aW_rot_ep_mat_el_ext,W_rot_ep_mat_el)
     !     
     !==============================================================================================
     ! Reciprocal coarse Wannier Representation to Real Wannier Representation: g(k,q) -> g(Re,Rp)
     !==============================================================================================
     ...
     ! 2) g(Re,q) -> g(Re,Rp)
     !
     allocate(g_Req_ext(nqmesh,nrr_k,num_wann,num_wann,3*nat))
     g_Req_ext=cmplx_0
     !
     write(*,*)'------------------------------------------------------------------'
     write(*,*)'------------------------------------------------------------------'
     ...
     write(*,*)'First step: g(k,q) -> g(Re,q)'
     !
     call G_kqw2Reqw_no_spin(nrr_k,irr_k,kmesh,g_kq_ext,g_Req_ext)
     !
     deallocate(g_kq_ext)
     !
     write(*,*)'------------------------------------------------------------------'
     ...
     write(*,*)'Second step: g(Re,q) -> g(Re,Rp)'
     !
     call G_Reqw2ReRpw_no_spin(nrr_q,nrr_k,irr_q,irr_k,qmesh,g_Req_ext,g_ReRp_ext)
     !
     deallocate(g_Req_ext)
     !
     write(*,*)'------------------------------------------------------------------'
     write(*,*)' '
     !
     ...
     g_realw_unit=find_free_unit()
     inquire(iolength=record_lengh) ag_ReRp
     open(unit=g_realw_unit,file=trim(trim(mesh_dir)//trim(g_realw_ext_file)),iostat=ierr,&
     form='unformatted',status='unknown',access='direct',recl=record_lengh)
     !
     do irq=1,nrr_q
        write(*,*)'Writing Real Wannier matrix elements irq = ',irq,'/',nrr_q
        write(unit=g_realw_unit,rec=irq,iostat=ierr) g_ReRp_ext(irq,:,:,:,:)
     enddo
     !
     close(unit=g_realw_unit)
  else
  ...
  !================================================================================
     ...
     do irq=1,nrr_q
        write(*,*)'Reading Real Wannier matrix elements irq = ',irq,'/',nrr_q
        read(unit=g_realw_unit,rec=irq,iostat=ierr) g_ReRp_ext(irq,:,:,:,:)
     enddo
     !
     close(unit=g_realw_unit)
     !
  ...
  if (iwant) then
     ...
     if (write_ph) then
        ...
        !
        !$omp parallel default(none) &
        !$omp shared(nqfmesh,qfmesh,nq1,nq2,nq3,U_ph,omega,frc,amass,ityp,nat) &
        !$omp private(iq,qpoint,dyn_q,w2,imode,jmode,na) 
        !
        !$omp do
        ! 
        do iq=1,nqfmesh
           ...
           do imode=1,3*nat
              ...
              if (w2(imode).gt.0.d0) then
                 !
                 omega(imode,iq)=sqrt(abs(w2(imode)))
                 !
              else
                 !
                 omega(imode,iq)=-sqrt(abs(w2(imode)))
              ...
              !
              omega(imode,iq)=omega(imode,iq)*0.001d0
              !
              do jmode=1,3*nat
                 !
                 na=((jmode-1)/3)+1
                 U_ph(jmode,imode,iq)=dyn_q(jmode,imode)/sqrt(amass(ityp(na)))
                 !
              enddo !jmode
              !
           enddo !imode
  ...
  !=========================================================================================
  allocate(g_Re(nrr_k,num_wann,num_wann,3*nat))
  allocate(g_wannier(num_wann,num_wann,3*nat))
  allocate(g_elph(num_wann,num_wann,3*nat))
  allocate(g_me(3*nat))
  !
  allocate(g_Re_ext(nrr_k,num_wann,num_wann,3*nat))
  allocate(g_wannier_ext(num_wann,num_wann,3*nat))
  allocate(g_elph_ext(num_wann,num_wann,3*nat))
  allocate(g_me_ext(3*nat))
  !
  !==========================================================================
...
!     trshld=1.9594 !C graph
  ...
  nqq=501
  !
  allocate(phonren(nqq,3*nat))
  phonren(:,:)=0.d0
  !
  ! We choose in which band and direction we will make the calculation and how
  ! many points will be in our calculation:
  !
  open(unit=1314,file=trim(trim(mesh_dir)//trim('path_phon.dat')))
  open(unit=1315,file=trim(trim(mesh_dir)//trim('path_phon_ren.dat')))
  !
  !==============================================================================================
  ! Calculation of the phonon mode (q,nu) adiabatic renormalization depending on doping
  ! through a given direction in the BZ (normaly a special direction)
  ...
  ! We know:
  !
  ! deltaomega(q,nu)=2*Sum_k_m_n[Re|g_ext(q,k,m,n,nu)*conjg(g_ext(q,k,m,n,nu))|*(f(E(k,n))-f(E(k+q,m)))/(E(k,n)-E(k+q,m))]
  !
  do iq=1,nqq
     !
...
!           sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2),eigen(:,iqpt)
     ...
     if (iq.lt.168) then
        !
        write(1314,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2),omega(:,iqpt)!abs(omega(:,iqpt))
        !
     elseif (iq.gt.167.and.iq.lt.251) then
        !
        write(1314,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           1.d0-sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2-1.d0/3.d0),omega(:,iqpt)!abs(omega(:,iqpt))
        !
     elseif (ik.gt.250) then
        !
        write(1314,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           1.d0+sqrt(1.d0/3.d0)-sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2),omega(:,iqpt)!abs(omega(:,iqpt))
        !
     endif
cycle
     !
     ...
     !
     ! We make the transformation: g(Re,Rp) -> g(Re,q)
     !
     g_Re(:,:,:,:)=cmplx_0
     g_Re_ext(:,:,:,:)=cmplx_0
     call G_ReRpw2Reqw_no_spin(nrr_k,nrr_q,irr_q,ndegen_q,qpoint,g_ReRp,g_Re)
     call G_ReRpw2Reqw_no_spin(nrr_k,nrr_q,irr_q,ndegen_q,qpoint,g_ReRp_ext,g_Re_ext)
     !
     ! We start the summ with respect to k (kmesh) and to n (jbnd) and m (ibnd). We limit this addition,
     ! we just take into account the states (k,m,n) we don't misprise
     !
     do ik=1,nkfmesh
        !
        ...
        g_wannier(:,:,:)=cmplx_0
        g_elph(:,:,:)=cmplx_0
        call G_Reqw2kqw_no_spin(nrr_k,irr_k,ndegen_k,kpoint,g_Re,g_wannier)
        call G_kqw2kqb_no_spin(U_el(:,:,ikpt_kq),U_el(:,:,ikpt_k),U_phon(:,:),g_wannier,g_elph)
        !
        g_wannier_ext(:,:,:)=cmplx_0
        g_elph_ext(:,:,:)=cmplx_0
        call G_Reqw2kqw_no_spin(nrr_k,irr_k,ndegen_k,kpoint,g_Re_ext,g_wannier_ext)
        call G_kqw2kqb_no_spin(U_el(:,:,ikpt_kq),U_el(:,:,ikpt_k),U_phon(:,:),g_wannier_ext,g_elph_ext)
        !
        !$omp parallel default(none) &
        !$omp shared(num_wann,eigen,trshld1,trshld2,ef,g_elph,g_elph_ext,nat,omega_phon,eps_6,phonren,ikpt_k,ikpt_kq,iq) &
        !$omp private(ibnd,jbnd,ekq,ekk,g_me,g_me_ext,imode,wqv,g2) 
        !
        !$omp do
        !
        do ibnd=1,num_wann
           !
           if (ibnd.lt.2) ekq=eigen(ibnd,ikpt_kq)+trshld1
           if (ibnd.gt.1) ekq=eigen(ibnd,ikpt_kq)+trshld2
           ! 
           do jbnd=1,num_wann
              !
              if (jbnd.lt.2) ekk=eigen(jbnd,ikpt_k)+trshld1
              if (jbnd.gt.1) ekk=eigen(jbnd,ikpt_k)+trshld2
              !
              if (ekk.lt.ef.and.ekq.gt.ef) then
                 !
                 g_me(:)=g_elph(ibnd,jbnd,:)
                 g_me_ext(:)=g_elph_ext(ibnd,jbnd,:)
                 !
!                 do imode=1,3*nat
                 do imode=1,3
                    !
                    ...
                    !
                    if (wqv.lt.eps_6) then
                       !
                       g2=0.0d0
                       !
                    else
                       !
!                       g2=weight_ph(wqv)*2*real(g_me_ext(imode)*g_me(imode))/(2.d0*wqv)
!                       g2=2*real(g_me_ext(imode)*conjg(g_me(imode)))/(2.d0*wqv)
                       g2=2*real(g_me(imode)*conjg(g_me(imode)))/(2.d0*wqv)
                       !
                    endif !wqv > 0
                    !
                    phonren(iq,imode)=phonren(iq,imode)+g2/(ekk-ekq)
                    !
                 enddo !imode
                 !
              endif ! ekk<ef, ekq>ef
...
!           endif !wqv > 0
!           !
!           phonren(iq,imode)=phonren(iq,imode)-g2/(eigen(1,ikpt_k)-eigen(2,ikpt_kq))
!           !
!        enddo !imode
!        !
!        g_me(:)=g_elph(1,2,:)
!        g_me_ext(:)=g_elph_ext(1,2,:)
!        !
!!        do imode=1,3*nat
!        do imode=1,3
!           !
...
!           endif !wqv > 0
!           !
!           phonren(iq,imode)=phonren(iq,imode)+g2/(eigen(2,ikpt_k)-eigen(1,ikpt_kq))
!           !
!        enddo !imode
        !
        !!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     enddo !ik
     !
     phonren(iq,:)=phonren(iq,:)/nkfmesh
     !
     if (iq.lt.168) then
        !
        write(1315,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2),abs(omega(:,iqpt))+phonren(iq,:)
        !
     elseif (iq.gt.167.and.iq.lt.251) then
        !
        write(1315,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           1.d0-sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2-1.d0/3.d0),abs(omega(:,iqpt))+phonren(iq,:)
        !
     elseif (ik.gt.250) then
        !
        write(1315,'(i10,3f8.5,f12.6,100f15.8)') iq,qpoint(:), &
           1.d0+sqrt(1.d0/3.d0)-sqrt(qpoint_cart(1)**2+qpoint_cart(2)**2+qpoint_cart(3)**2),abs(omega(:,iqpt))+phonren(iq,:)
        !
     endif
     !
  enddo !iq
