program lambda_full_nk
  ...
  !energy
  integer :: epts,jpts,ifile
  real(dp) :: e_min,e_max,d_e,e
  real(dp),allocatable :: lambda(:),hlambda(:,:),dose(:)
  ...
  dose(:)=0.d0
  !
  !We read the hlambda function in all the files we have got
  !
  do ifile=1,50
     !
     do ipts=ifile*40-39,ifile*40
        do jpts=1,wpts
           !
           read(1000+ifile) hlambda(ipts,jpts)
           !
        enddo !jpts
     enddo !ipts
     !
  ...
  enddo !ifile
  !
  !We read the dos file
  !
  open(unit=1314,file=trim(trim(mesh_dir)//trim('bands_DOS/dos_elec.bin')),form='unformatted')
  !
  do ipts=1,epts
     !
     read(1314) dose(ipts)
...
write(*,*) dose(:)
  !
  ! We calculate lambda in function of the energy
  !
  do ipts=1,epts
     !
     e=e_min+(ipts-1)*d_e
     !
     call hlambda_calculate_full_t0(wpts,w_min,w_max,d_w,hlambda(ipts,:),lambda(ipts))
     lambda(ipts)=2*lambda(ipts)/dose(ipts)
     !
!     write(1314,'(f18.6,f10.3)') e,lambda
     write(1314,*) e,lambda(ipts)*10000
     !
  enddo !ipts
