program eliashberg_full
  ...
  real(dp),allocatable :: eliash_fplus_so(:,:,:),eliash_fminus_so(:,:,:)
...
!THIS IS FOR AVERAGE PROGRAM
  ...
  allocate(g_me(nspin,nspin,3*nat))
  !
  !==========================================================================
  ...
  w_min=0.0d0
  w_max=0.065d0
  wpts=3251 ! how many points for our w variable in eliashberg function a^2F(w)
  !
  if (lspinorb) then
     !
     allocate(eliash_fplus_so(wpts,npol,npol))
     allocate(eliash_fminus_so(wpts,npol,npol))
     !
  endif
  !
  allocate(eliash_fplus(wpts))
  allocate(eliash_fminus(wpts))
  !
  ...
  nkk=501
  !
  open(unit=1314,file=trim(trim(mesh_dir)//trim('path.dat')))
  !
  !==============================================================================================
  ! Calculation of the state dependent Eliashberg +/- Function in fonction the
  ...
  ! We know:
  !
  ! a^2F+/-(k,n,si,sj,w)=Sum_q_m_nu[|g(q,k,m,n,si,sj,nu)|^2*Delta[w-w(q,nu)]*Delta[E(k,n)-E(k+q,m)+/-w]]
  !
  ...
  !
!  call force_linearity_G(nqf1,nqf2,nqf3,qfmesh,omega)
  !
  ...
  do ik=1,nkk
     ...
     if (ik.lt.251) then
        ...
        dkvec(3)=0.d0
        !
        kpoint(1)=0.250d0+dkvec(1)*(ik-1)
        kpoint(2)=0.0d0+dkvec(2)*(ik-1)
        kpoint(3)=0.0d0+dkvec(3)*(ik-1)
        !
!        if (ik.gt.160) then
!           !
           sigma_e=0.001
!           !
!        elseif (ik.le.160.and.ik.gt.120) then
!           !
!           sigma_e=2.d0*sigma_w
!           !
!        elseif (ik.le.120) then
!           !
!           sigma_e=3.d0*sigma_w
!           !
!        endif
        !
     else
        ...
        dkvec(3)=0.d0
        !
        kpoint(1)=0.0d0+dkvec(1)*(ik-251)
        kpoint(2)=0.0d0+dkvec(2)*(ik-251)
        kpoint(3)=0.0d0+dkvec(3)*(ik-251)
        !
!        if (ik.lt.341) then
!           !
           sigma_e=0.001
!           !
!        elseif (ik.ge.341.and.ik.lt.301) then
!           !
!           sigma_e=2.d0*sigma_w
!           !
!        elseif (ik.ge.301) then
!           !
!           sigma_e=3.d0*sigma_w
!           !
!        endif
     ...
     !
     write(1314,'(i10,3f8.5,f12.6,100f15.8)') ik,kpoint(:), &
     ...
     if (lspinorb) then
        !
        eliash_fplus_so(:,:,:)=0.0d0
        eliash_fminus_so(:,:,:)=0.0d0
        !
     endif
     !
     eliash_fplus(:)=0.0d0
     ...
     do iq_2=1,nqfmesh_2
        ...
        !
        g_me(:,:,:)=g_elph(ibnd,jbnd,:,:,:)
        !
        if (lspinorb) call add_q_m_nu_term_full_so(w_min,w_max,wpts,sigma_w,sigma_e,ekk,ekq,omega_phon,g_me(:,:,:),eliash_fplus_so,eliash_fminus_so,3*nat-4)
        !
        call add_q_m_nu_term_full(w_min,w_max,wpts,sigma_w,sigma_e,ekk,ekq,omega_phon,g_me(:,:,:),eliash_fplus,eliash_fminus,3*nat-4)
        !
     enddo !iq_2
     !
     ...
     if (lspinorb) then 
        !
        eliash_fplus_so(:,:,:)=eliash_fplus_so(:,:,:)/nqfmesh
        eliash_fminus_so(:,:,:)=eliash_fminus_so(:,:,:)/nqfmesh
        !
     endif
     !
     eliash_fplus(:)=eliash_fplus(:)/nqfmesh
     eliash_fminus(:)=eliash_fminus(:)/nqfmesh
     !
     ...
     if (lspinorb) then
        ...
        do ifs=1,nfs
           ...
           !
           if (ik<10) then
              open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_so_')//trim(ik_loc1)//trim('_')//trim(ifs_loc)//trim('.dat')))
              open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_so_')//trim(ik_loc1)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
              open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_so_')//trim(ik_loc1)//trim('_')//trim(ifs_loc)//trim('.dat')))
              open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_so_')//trim(ik_loc1)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<100) then
              open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_so_')//trim(ik_loc2)//trim('_')//trim(ifs_loc)//trim('.dat')))
              open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_so_')//trim(ik_loc2)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
              open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_so_')//trim(ik_loc2)//trim('_')//trim(ifs_loc)//trim('.dat')))
              open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_so_')//trim(ik_loc2)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<1000) then
              open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_so_')//trim(ik_loc3)//trim('_')//trim(ifs_loc)//trim('.dat')))
              open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_so_')//trim(ik_loc3)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
              open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_so_')//trim(ik_loc3)//trim('_')//trim(ifs_loc)//trim('.dat')))
              open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_so_')//trim(ik_loc3)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<10000) then
              open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_so_')//trim(ik_loc4)//trim('_')//trim(ifs_loc)//trim('.dat')))
              open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_so_')//trim(ik_loc4)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
              open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_so_')//trim(ik_loc4)//trim('_')//trim(ifs_loc)//trim('.dat')))
              open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_so_')//trim(ik_loc4)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           endif
           !
           do ipts=1,wpts
              !
              if (ifs==1) then
                 !
                 write(10*ik+1,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fplus_so(ipts,1,1)
                 write(10*ik+100000+1) eliash_fplus_so(ipts,1,1)
                 write(10*ik+2,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fminus_so(ipts,1,1)
                 write(10*ik+100000+2) eliash_fminus_so(ipts,1,1)
                 !
              elseif (ifs==2) then
                 !
                 write(10*ik+1,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fplus_so(ipts,1,2)
                 write(10*ik+100000+1) eliash_fplus_so(ipts,1,2)
                 write(10*ik+2,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fminus_so(ipts,1,2)
                 write(10*ik+100000+2) eliash_fminus_so(ipts,1,2)
                 !
              elseif (ifs==3) then
                 !
                 write(10*ik+1,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fplus_so(ipts,2,1)
                 write(10*ik+100000+1) eliash_fplus_so(ipts,2,1)
                 write(10*ik+2,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fminus_so(ipts,2,1)
                 write(10*ik+100000+2) eliash_fminus_so(ipts,2,1)
                 !
              elseif (ifs==4) then
                 !
                 write(10*ik+1,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fplus_so(ipts,2,2)
                 write(10*ik+100000+1) eliash_fplus_so(ipts,2,2)
                 write(10*ik+2,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fminus_so(ipts,2,2)
                 write(10*ik+100000+2) eliash_fminus_so(ipts,2,2)
                 !
              endif
              !
           enddo !ipts
           !
           close(10*ik+1)
           close(10*ik+100000+1)
           close(10*ik+2)
           close(10*ik+100000+2)
           !
        enddo !ifs
        !
     endif !lspinorb
     !
     if (ik<10) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc1)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc1)//trim('.dat')))
        open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
     elseif (ik<100) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc2)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc2)//trim('.dat')))
        open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
     elseif (ik<1000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc3)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc3)//trim('.dat')))
        open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
     elseif (ik<10000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc4)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc4)//trim('.dat')))
        open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
     endif
     !
     do ipts=1,wpts
        !
        write(10*ik+1,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fplus(ipts)
        write(10*ik+100000+1) eliash_fplus(ipts)
        write(10*ik+2,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fminus(ipts)
        write(10*ik+100000+2) eliash_fminus(ipts)
        !
     enddo !ipts
     !
     close(10*ik+1)
