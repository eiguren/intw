program imaginary_selfen
  ...
  integer                  :: ep_unit, Wep_unit, ep_ind_unit
  logical                  :: q_points_consistent
  complex(dp), allocatable :: aep_mat_el(:,:,:,:,:,:),aep_mat_el_ind(:,:,:,:,:,:),ep_mat_el(:,:,:,:,:,:)
  complex(dp), allocatable :: aep_mat_el_2(:,:,:,:),aep_mat_el_2_ind(:,:,:,:)
  ...
  complex(dp) :: g1
  real(dp) :: g2
  complex(dp),allocatable :: g_kq(:,:,:,:),g_elph(:,:,:,:)
  complex(dp),allocatable :: g_kq_ind(:,:,:,:),g_elph_ind(:,:,:,:)
  ...
  !
!  allocate (aep_mat_el(nk1*nk2*nk3,nbands_loc,nbands_loc,nspin,nspin,3*nat))  ! q bakoitzerako guztia
!  allocate (aep_mat_el_2(nk1*nk2*nk3,nbands_loc,nbands_loc,3*nat))
!  allocate (aep_mat_el_ind(nk1*nk2*nk3,nbands_loc,nbands_loc,nspin,nspin,3*nat))  ! q bakoitzerako guztia
!  allocate (aep_mat_el_2_ind(nk1*nk2*nk3,nbands_loc,nbands_loc,3*nat))
!  allocate (ep_mat_el(nk1*nk2*nk3,nbands_loc,nbands_loc,nspin,nspin,3*nat))
  !
  allocate (aep_mat_el(nk1*nk2*nk3,4,4,1,1,3*nat))  ! q bakoitzerako guztia
  allocate (aep_mat_el_2(nk1*nk2*nk3,3,3,3*nat))
  allocate (aep_mat_el_ind(nk1*nk2*nk3,4,4,1,1,3*nat))  ! q bakoitzerako guztia
  allocate (aep_mat_el_2_ind(nk1*nk2*nk3,3,3,3*nat))
  allocate (ep_mat_el(nk1*nk2*nk3,4,4,1,1,3*nat))
  aep_mat_el(:,:,:,:,:,:)=cmplx_0
  aep_mat_el_2(:,:,:,:)=cmplx_0
  aep_mat_el_ind(:,:,:,:,:,:)=cmplx_0
  aep_mat_el_2_ind(:,:,:,:)=cmplx_0
  ep_mat_el(:,:,:,:,:,:)=cmplx_0
  !
  !================================================================================
  ...
  !
!  allocate(g_kq(nkmesh,num_bands,num_bands,3*nat))
!  allocate(g_kq_ind(nkmesh,num_bands,num_bands,3*nat))
  allocate(g_kq(nkmesh,3,3,3*nat))
  allocate(g_kq_ind(nkmesh,3,3,3*nat))
  g_kq(:,:,:,:)=cmplx_0
  g_kq_ind(:,:,:,:)=cmplx_0
  !
  ...
  open(unit=ep_unit,&
  file=trim(trim(mesh_dir)//trim(ep_mat_file)//trim('_K_scal')),&
!  file=trim(trim(mesh_dir)//trim(ep_mat_file)//trim('_K_so')),&
!  file=trim(trim(mesh_dir)//trim('EP_DATA_IND/')//trim(ep_mat_file)//trim('_K')), &
  iostat=ierr,form='unformatted',status='unknown',access='direct',recl=record_lengh)
  !
  ep_ind_unit=find_free_unit()
  inquire(iolength=record_lengh) ep_mat_el
  open(unit=ep_ind_unit,&
!  file=trim(trim(mesh_dir)//trim('EP_DATA_IND/')//trim(ep_mat_file)//trim('_K_IND')),&
  file=trim(trim(mesh_dir)//trim(ep_mat_file)//trim('_K_IND_scal')),&
!  file=trim(trim(mesh_dir)//trim(ep_mat_file)//trim('_K_IND_so')),&
  iostat=ierr,form='unformatted',status='unknown',access='direct',recl=record_lengh)
  !
  read(unit=ep_unit,rec=1,iostat=ierr) aep_mat_el(:,:,:,:,:,:)
  read(unit=ep_ind_unit,rec=1,iostat=ierr) aep_mat_el_ind(:,:,:,:,:,:)
  !
  close(unit=ep_unit)
  close(unit=ep_ind_unit)
  !
  do ik=1,nkmesh
     do imode=1,3*nat
!        do ibnd=1,num_bands
...
!           do jbnd=1,num_bands
           do jbnd=1,3
              ...
              if (nspin.gt.1) then
                 !
                 aep_mat_el_2(ik,ibnd,jbnd,imode)=aep_mat_el(ik,ibnd,jbnd,1,1,imode)+aep_mat_el(ik,ibnd,jbnd,1,2,imode)+ &
                                                  aep_mat_el(ik,ibnd,jbnd,2,1,imode)+aep_mat_el(ik,ibnd,jbnd,2,2,imode)
                 !
                 aep_mat_el_2_ind(ik,ibnd,jbnd,imode)=aep_mat_el_2(ik,ibnd,jbnd,imode)- &
                                                      (aep_mat_el_ind(ik,ibnd,jbnd,1,1,imode)+aep_mat_el_ind(ik,ibnd,jbnd,1,2,imode)+ &
                                                      aep_mat_el_ind(ik,ibnd,jbnd,2,1,imode)+aep_mat_el_ind(ik,ibnd,jbnd,2,2,imode))
                 !
              else
                 !
                 aep_mat_el_2(ik,ibnd,jbnd,imode)=aep_mat_el(ik,ibnd,jbnd,1,1,imode)
                 aep_mat_el_2_ind(ik,ibnd,jbnd,imode)=aep_mat_el_2(ik,ibnd,jbnd,imode)-aep_mat_el_ind(ik,ibnd,jbnd,1,1,imode)
                 !
              endif
              !
           enddo !jbnd
  ...
  !
  g_kq(:,:,:,:)=aep_mat_el_2(:,:,:,:)
  g_kq_ind(:,:,:,:)=aep_mat_el_2_ind(:,:,:,:)
  !
  deallocate(aep_mat_el,aep_mat_el_2,ep_mat_el,aep_mat_el_ind,aep_mat_el_2_ind)
  !     
  !====================================================================================
...
!     trshld=1.6087
  ...
  call switch_indices(nqf1,nqf2,nqf3,iq,i,j,k,+1)
  qpoint(:)=qfmesh(:,iq)
  !
  call DM_Rp2q(nrr_q,irr_q,ndegen_q,qpoint,dyn_r,U_phon(:,:),omega_phon(:))
  !
  ef=-trshld
  !
  sumg1(:)=0.d0
  sumg2(:)=0.d0
  !
!  allocate(g_elph(nkmesh,num_bands,num_bands,3*nat))
!  allocate(g_elph_ind(nkmesh,num_bands,num_bands,3*nat))
  allocate(g_elph(nkmesh,3,3,3*nat))
  allocate(g_elph_ind(nkmesh,3,3,3*nat))
  !
  g_elph(:,:,:,:)=cmplx_0
  g_elph_ind(:,:,:,:)=cmplx_0
  !
...
!        do jbnd=1,num_bands
        do jbnd=1,3
           ...
           do imode=1,3*nat
              do jmode=1,3*nat
                 !
                 g_elph(ik,ibnd,jbnd,imode)=g_elph(ik,ibnd,jbnd,imode)+ &
                                      U_phon(jmode,imode)*g_kq(ik,ibnd,jbnd,jmode)
                 !
                 g_elph_ind(ik,ibnd,jbnd,imode)=g_elph_ind(ik,ibnd,jbnd,imode)+ &
                                      U_phon(jmode,imode)*g_kq_ind(ik,ibnd,jbnd,jmode)
                 !
              enddo !jmode
           enddo !imode
           !
...
!     do ibnd=1,num_bands
     do ibnd=2,3
        ...
        ekq=QE_eig_kq(ibnd)
        !
!        if (ibnd==1.or.ibnd==2) ekq=QE_eig_kq(1)
!        if (ibnd==3.or.ibnd==4) ekq=QE_eig_kq(2)
!        if (ibnd==5.or.ibnd==6) ekq=QE_eig_kq(3)
        !
!        do jbnd=1,num_bands
        do jbnd=2,3
           !
           ekk=QE_eig_k(jbnd)
           !
!           if (jbnd==1.or.jbnd==2) ekk=QE_eig_k(1)
!           if (jbnd==3.or.jbnd==4) ekk=QE_eig_k(2)
!           if (jbnd==5.or.jbnd==6) ekk=QE_eig_k(3)
           !
           if (ekk.lt.ef.and.ekq.gt.ef) then
              !
!              g2=2*abs(conjg(g_elph(ik,ibnd,jbnd,1))*g_elph_ind(ik,ibnd,jbnd,1))
              g2=2*real(conjg(g_elph(ik,ibnd,jbnd,1))*g_elph_ind(ik,ibnd,jbnd,1))
              !
!              if (jbnd.lt.3.and.ibnd.gt.4) then
!                 !
!                 g2=0.5*abs(conjg(g_elph(ik,3,1,1))*g_elph_ind(ik,3,1,1))
!                 !
!              elseif (jbnd.gt.2.and.jbnd.lt.5.and.ibnd.gt.4) then
!                 !
!                 g2=0.5*abs(conjg(g_elph(ik,3,2,1))*g_elph_ind(ik,3,2,1))
!                 !
!              endif
              !
!              sumg1(ik)=sumg1(ik)+g2/(ekk-ekq)
...
!              g2=2*abs(conjg(g_elph(ik,ibnd,jbnd,1))*g_elph_ind(ik,ibnd,jbnd,1))
              g2=2*real(conjg(g_elph(ik,ibnd,jbnd,1))*g_elph_ind(ik,ibnd,jbnd,1))
              !
!              if (ibnd.lt.3.and.jbnd.gt.4) then
!                 !
!                 g2=0.5*abs(conjg(g_elph(ik,1,3,1))*g_elph_ind(ik,1,3,1))
!                 !
!              elseif (ibnd.gt.2.and.ibnd.lt.5.and.jbnd.gt.4) then
!                 !
!                 g2=0.5*abs(conjg(g_elph(ik,2,3,1))*g_elph_ind(ik,2,3,1))
!                 !
!              endif
              !
!              sumg2(ik)=sumg2(ik)+g2/(ekq-ekk)
              sumg2(ik)=sumg2(ik)+g2
              !
           endif
           !
        enddo !jbnd
  ...
  !
  open(unit=11,file=trim(trim(mesh_dir)//trim('diff_tex.dat_K_paper_mode1_A')))
  open(unit=22,file=trim(trim(mesh_dir)//trim('diff_tex.dat_K_paper_mode1_B')))
!  open(unit=11,file=trim(trim(mesh_dir)//trim('g_tex.dat_K_IND')))
!  open(unit=22,file=trim(trim(mesh_dir)//trim('g_zati_w_tex.dat_K_IND')))
  !
  do ikx=-nk1,nk1
     do iky=-nk2,nk2
        !
