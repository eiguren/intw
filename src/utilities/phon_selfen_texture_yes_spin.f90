program imaginary_selfen
  ...
  !=========================================================================================
  allocate(g_Re(nrr_k,num_wann,num_wann,nspin,nspin,3*nat))
...
!  do imode=1,3*nat
!     !
!     write(8888,'(a,f20.12)') 'Energy = ',omega_phon(imode)
!     write(8888,*)
!     write(8888,'(a)') 'Polarization : '
!     !
!     do jmode=1,3*nat
!        !
!        write(8888,'(100f20.12)') U_phon(jmode,imode)
!        !
!     enddo
!     !
!  enddo
...
!  stop
  ...
  !
  g_Re(:,:,:,:,:,:)=cmplx_0
  call G_ReRpw2Reqw(nrr_k,nrr_q,irr_q,ndegen_q,qpoint,g_ReRp,g_Re)
  !
  !$omp parallel default(none) &
  !$omp shared(nkfmesh,kfmesh,nkf1,nkf2,nkf3,qpoint,eigen,cmplx_0,g_Re,nrr_k,irr_k,ndegen_k,wqv,ef,eps_5,eps_6,nspin) &
  !$omp shared(sumg1,sumg2,U_el,U_phon,imode,num_wann) &
  !$omp private(ik,kpoint,i,j,k,kpoint_in_1bz,kqpoint_in_1bz,GKQ_bz,ikpt_k,ikpt_kq,ibnd,jbnd,ekk,ekq,g_wannier,g_elph,g2,g1) 
  !
  !$omp do
  !
  do ik=1,nkfmesh
     ...
     g_wannier(:,:,:,:,:)=cmplx_0
     g_elph(:,:,:,:,:)=cmplx_0
     call G_Reqw2kqw(nrr_k,irr_k,ndegen_k,kpoint,g_Re,g_wannier)
...
!        do jbnd=1,num_wann
        do jbnd=3,6
           ...
           if (ekk.lt.ef.and.ekq.gt.ef) then
              ...
              if (nspin.gt.1) then
                 !
                 g1=g_elph(ibnd,jbnd,1,1,1)+g_elph(ibnd,jbnd,1,2,1)+ &
                    g_elph(ibnd,jbnd,2,1,1)+g_elph(ibnd,jbnd,2,2,1)
                 !
                 g2=(abs(g1)**2.d0)
                 !
              else
                 !
                 g2=(abs(g_elph(ibnd,jbnd,1,1,1))**2.d0)
                 !
              endif
              !
              sumg1(ik)=g2
  ...
  !
  open(unit=11,file=trim(trim(mesh_dir)//trim('g_tex.dat')))
  open(unit=22,file=trim(trim(mesh_dir)//trim('g_zati_w_tex.dat')))
  !
  do ikx=-nkf1,nkf1
     do iky=-nkf2,nkf2
        !
