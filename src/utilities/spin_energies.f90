program spin_texture
  ...
  !spin components
  integer :: ikx,iky
  real(dp),allocatable :: spin_m(:,:,:),spin_kb(:,:)
  ...
  call init_us_1
  !
  allocate(spin_kb(nkmesh,nbands_loc))
  allocate(spin_m(-nk1:nk1,-nk2:nk2,nbands_loc))
  spin_kb(:,:)=0.d0
  spin_m(:,:,:)=0.d0
  !
  do ik=1,nkmesh
  ...
  do ibnd=1,num_bands
     ...
     do ik=1,nkmesh
        ...
        call find_k_1BZ_and_G(kpoint,nk1,nk2,nk3,i,j,k,kpoint_in_1bz,GKQ_bz)
        !
        spin_m((i-1)-nk1,(j-1)-nk2,ibnd)=spin_kb(ik,ibnd)
        spin_m((i-1)-nk1,j-1,ibnd)=spin_kb(ik,ibnd)
        spin_m(i-1,(j-1)-nk2,ibnd)=spin_kb(ik,ibnd)
        spin_m(i-1,j-1,ibnd)=spin_kb(ik,ibnd)
        !
     enddo !ik
     !
     do ik=-nk1,nk1-1
        !
        spin_m(ik,nk2,ibnd)=spin_m(ik,0,ibnd)
        !
     enddo !ik
     !
     do ik=-nk2,nk2-1
        !
        spin_m(nk1,ik,ibnd)=spin_m(0,ik,ibnd)
        !
     enddo !ik
     !
     spin_m(nk1,nk2,ibnd)=spin_m(0,0,ibnd)
     !
  enddo !ibnd
  !
  open(unit=11,file=trim(trim(mesh_dir)//trim('spin_energies.dat')))
  !
  do ikx=-nk1,nk1
     do iky=-nk2,nk2
        !
        ...
        !
        write(11,'(100f16.8)') kpoint(1),kpoint(2),(spin_m(ikx,iky,ibnd),ibnd=1,num_bands)
        !
     enddo !iky
  enddo !ikx
  !
