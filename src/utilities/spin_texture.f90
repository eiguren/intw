program spin_texture
  ...
  !local/aux variables
  real(dp)                 :: sigma_w,term,sigma_e,tita,alpha
  integer                  :: nbands_loc,irk,irq,ikq
  integer                  :: i,j,k
  integer                  :: ii,jj,kk
  integer                  :: jr,kr
  ...
  do ik=1,nkmesh
     ...
     do ibnd=1,num_bands
        ...
        do iG=1,nG_max
           !
           spin_kb(ik,ibnd,1)=spin_kb(ik,ibnd,1)+2.d0*real(conjg(wfc_k(iG,ibnd,1))*wfc_k(iG,ibnd,2))
           spin_kb(ik,ibnd,2)=spin_kb(ik,ibnd,2)+2.d0*aimag(conjg(wfc_k(iG,ibnd,1))*wfc_k(iG,ibnd,2))
           spin_kb(ik,ibnd,3)=spin_kb(ik,ibnd,3)+abs(wfc_k(iG,ibnd,1))**2.d0-abs(wfc_k(iG,ibnd,2))**2.d0
           spin_kb(ik,ibnd,4)=spin_kb(ik,ibnd,4)+abs(wfc_k(iG,ibnd,1))**2.d0+abs(wfc_k(iG,ibnd,2))**2.d0
           !
        enddo
           !
     enddo !ibnd
...
!     call get_spin_component_convolution(list_iGk,wfc_k,spin_kb(ik,1:nbands_loc,1:3))
     !
     if (abs(QE_eig_k(1)-QE_eig_k(2)).lt.0.005) then
        !
!        spin_kb(ik,1,:)=cmplx_0
!        spin_kb(ik,2,:)=cmplx_0
        spin_kb(ik,1,:)=sqrt(1.d0/2.d0)*(spin_kb(ik,1,:)+spin_kb(ik,2,:))
        spin_kb(ik,2,:)=spin_kb(ik,1,:)
        !
     endif
     !
     if (abs(QE_eig_k(3)-QE_eig_k(4)).lt.0.005) then
        !
!        spin_kb(ik,3,:)=cmplx_0
!        spin_kb(ik,4,:)=cmplx_0
        spin_kb(ik,3,:)=sqrt(1.d0/2.d0)*(spin_kb(ik,3,:)+spin_kb(ik,4,:))
        spin_kb(ik,4,:)=spin_kb(ik,3,:)
        !
     endif
     !
     if (abs(QE_eig_k(1)-QE_eig_k(2)).lt.0.001) then
        !
        spin_kb(ik,1,:)=cmplx_0
        spin_kb(ik,2,:)=cmplx_0
        !
     ...
     !
     if (abs(QE_eig_k(3)-QE_eig_k(4)).lt.0.001) then
        !
        spin_kb(ik,3,:)=cmplx_0
        spin_kb(ik,4,:)=cmplx_0
        !
  ...
  do ik1=2,nk1
     do ik2=2,nk2
        !
        if (ik2.le.nk2-(ik1-1)) then
           !
           if (sign(1.0,real(spin_kb_xy(ik1,ik2,1,3))).ne.sign(1.0,real(spin_kb_xy((nk1/3)+1,(nk2/3)+1,1,3)))) then
              !
              a=spin_kb_xy(ik1,ik2,1,3)
              b=spin_kb_xy(ik1,ik2,2,3)
              !
              spin_kb_xy(ik1,ik2,1,3)=b
              spin_kb_xy(ik1,ik2,2,3)=a
              !
           endif
           !
           if (sign(1.0,real(spin_kb_xy(ik1,ik2,3,3))).ne.sign(1.0,real(spin_kb_xy((nk1/3)+1,(nk2/3)+1,3,3)))) then
              !
              a=spin_kb_xy(ik1,ik2,3,3)
              b=spin_kb_xy(nk1-(ik2-2),nk2-(ik1-2),3,3)
              !
              spin_kb_xy(ik1,ik2,3,3)=b
              spin_kb_xy(nk1-(ik2-2),nk2-(ik1-2),3,3)=a
              !
           endif
           !
           if (sign(1.0,real(spin_kb_xy(ik1,ik2,4,3))).ne.sign(1.0,real(spin_kb_xy((nk1/3)+1,(nk2/3)+1,4,3)))) then
              !
              a=spin_kb_xy(ik1,ik2,4,3)
              b=spin_kb_xy(nk1-(ik2-2),nk2-(ik1-2),4,3)
              !
              spin_kb_xy(ik1,ik2,4,3)=b
              spin_kb_xy(nk1-(ik2-2),nk2-(ik1-2),4,3)=a
              !
           endif
           !
        else
           !
           if (sign(1.0,real(spin_kb_xy(ik1,ik2,1,3))).ne.sign(1.0,real(spin_kb_xy(nk1+1-(nk1/3),nk2+1-(nk2/3),1,3)))) then
              !
              a=spin_kb_xy(ik1,ik2,1,3)
              b=spin_kb_xy(ik1,ik2,2,3)
              !
  ...
  do ibnd=1,num_bands
     ...
     !
     open(unit=11,file=trim(trim(mesh_dir)//trim('spin_true_fine_')//(trim(ibnd_loc))//trim('.dat')))
     !
     do ikx=-nk1,nk1
        do iky=-nk2,nk2
           !
           ...
           !
!           spin_m_tot(ikx,iky,ibnd)=sqrt(spin_m(ikx,iky,ibnd,1)**2.d0+spin_m(ikx,iky,ibnd,2)**2.d0+spin_m(ikx,iky,ibnd,3)**2.d0)
           !
...
!                 spin_m_tot(ikx,iky,ibnd), &
!                 sqrt(spin_m(ikx,iky,ibnd,1)**2.d0+spin_m(ikx,iky,ibnd,2)**2.d0+spin_m(ikx,iky,ibnd,3)**2.d0),&
!                 spin_m(ikx,iky,ibnd,4)
           !
        enddo !iky
     enddo !ikx
     !
...
!     write(ibnd_loc,'(i1)')ibnd
!     !
!     open(unit=11,file=trim(trim(mesh_dir)//trim('grad_spin_')//(trim(ibnd_loc))//trim('.dat')))
!     !
!     do ikx=-nk1+1,nk1-1
!        do iky=-nk2+1,nk2-1
!           !
...
!           !
!           !
!           grad_spin(ikx,iky,ibnd)=(spin_m_tot(ikx+1,iky,ibnd)-spin_m_tot(ikx-1,iky,ibnd))/real(2.d0/nk1)+ &
!                                   (spin_m_tot(ikx,iky+1,ibnd)-spin_m_tot(ikx,iky-1,ibnd))/real(2.d0/nk2)
!           !
!           write(11,'(100f16.8)') kpoint(1),kpoint(2),grad_spin(ikx,iky,ibnd)
!           !
!        enddo !ikx
!     enddo !iky
!  enddo !ibnd
