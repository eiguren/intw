program spectral_function_nk
  ...
  real(dp)                 :: kpoint(3), kpoint_cart(3), kqpoint(3), kqpoint_in_1bz(3), k_K(3),k_K_cart(3)
  ...
  real(dp) :: trshld,e_min,e_max,d_e,ef,dkvec(3),ekk
  real(dp),allocatable :: reselfen_w(:),imselfen_w(:),a_w(:)
  ...
  !
  !================================================================================
  ! Starting the el-ph coupling parameter lambda in Quasi Elastic approx.
  ! and state dependent, that is through a k pathway we did when we calculated
  ! the eliashber qe functions we are going to use
  !================================================================================
  !
  !================================================================================
...
!     dkvec(2)=0.0005d0
     ...
     !
!     kpoint(1)=0.66675d0+dkvec(1)*(ik-265)
!     kpoint(2)=0.66675d0+dkvec(2)*(ik-265)
!     kpoint(3)=0.0000d0+dkvec(3)*(ik-265)
     kpoint(1)=0.d0+dkvec(1)*(ik-1)
     kpoint(2)=0.d0+dkvec(2)*(ik-1)
     kpoint(3)=0.d0+dkvec(3)*(ik-1)
     !
     ...
     !
     ! We read our imaginary self-energies for each state (k,n) and in fonction of quasiparticle energy (w or e)
     !
     ...
     !
     if (ik<10) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('im_selfen_GM_down/im_selfen_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('re_selfen_GM_down/re_selfen_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
     elseif (ik<100) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('im_selfen_GM_down/im_selfen_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('re_selfen_GM_down/re_selfen_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
     elseif (ik<1000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('im_selfen_GM_down/im_selfen_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('re_selfen_GM_down/re_selfen_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
     elseif (ik<10000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('im_selfen_GM_down/im_selfen_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('re_selfen_GM_down/re_selfen_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
     endif
     !
     do ipts=1,epts
        !
        read(10*ik+1) imselfen_w(ipts)
        read(10*ik+2) reselfen_w(ipts)
        !
     enddo !ipts
     !
     close(10*ik+1)
     close(10*ik+2)
     !
     imselfen_w(:)=imselfen_w(:)+d_e
     !
     call spectral_function_im_re_selfen(epts,e_min,e_max,d_e,ekk,ef,imselfen_w,reselfen_w,a_w)
     !
!     a_w(:)=a_w(:)*epts/sum(a_w(:))
     !
     do ipts=1,epts
        !
!        write(1314,'(100f16.8)') sqrt(k_K_cart(1)**2.d0+k_K_cart(2)**2.d0+k_K_cart(3)**2.d0)+ &
!                                 sqrt((kpoint_cart(1)-k_K_cart(1))**2.d0+(kpoint_cart(2)-k_K_cart(2))**2.d0+(kpoint_cart(3)-k_K_cart(3))**2.d0),&
!                                 e_min+d_e*(ipts-1),a_w(ipts)
        !
        write(1315,'(100f16.8)') sqrt(kpoint_cart(1)**2.d0+kpoint_cart(2)**2.d0+kpoint_cart(3)**2.d0),e_min+d_e*(ipts-1),a_w(ipts)
        !
     enddo
     !
  enddo !nkk

end program spectral_function_nk
