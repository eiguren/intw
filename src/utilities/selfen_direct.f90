program selfen_direct_nk
  ...
  real(dp) :: trshld,ef,e_min,e_max,d_e,dkvec(3)
  real(dp) :: ekq,ekk,delta_eplus,delta_eminus,wqv,e
  real(dp),allocatable :: reselfen_w(:),imselfen_w(:)
  real(dp),allocatable :: reselfen_w_2(:),imselfen_w_2(:)
...
!THIS IS FOR AVERAGE PROGRAM
  ...
  allocate(reselfen_w_2(epts))
  allocate(imselfen_w_2(epts))
  !
  reselfen_w(:)=0.d0
  imselfen_w(:)=0.d0
  reselfen_w_2(:)=0.d0
...
!     do ibnd=1,num_wann
     do ibnd=3,4
        ...
        do imode=1,3*nat
           ...
           !
           !$omp parallel default(none) &
           !$omp shared(epts,e_min,d_e,sigma_e,ekq,wqv,ekk,trshld,reselfen_w,imselfen_w,reselfen_w_2,imselfen_w_2,g2,pi,ef) &
           !$omp private(ipts,e) 
           !
           !$omp do
           !
           do ipts=1,epts
              ...
              e=e_min+d_e*(ipts-1)
              !
              if (e+ekk.gt.ekq.and.ekq.gt.ef) then
                 !
!                 reselfen_w(ipts)=reselfen_w(ipts)+g2*(e-wqv-(ekq-ekk))/((e-wqv-(ekq-ekk))**2.d0+(sigma_e)**2.d0)
                 !
!                 imselfen_w(ipts)=imselfen_w(ipts)+g2*sigma_e/((e-wqv-(ekq-ekk))**2.d0+(sigma_e)**2.d0)
                 imselfen_w(ipts)=imselfen_w(ipts)+g2*pi*exp(-((e-wqv-(ekq-ekk))/sigma_e)**2.d0)/(sigma_e*sqrt(pi))
                 !
              elseif (e+ekk.lt.ekq.and.ekq.lt.ef) then
                 !
!                 reselfen_w(ipts)=reselfen_w(ipts)+g2*(e+wqv-(ekq-ekk))/((e+wqv-(ekq-ekk))**2.d0+(sigma_e)**2.d0)
                 !
!                 imselfen_w(ipts)=imselfen_w(ipts)+g2*sigma_e/((e+wqv-(ekq-ekk))**2.d0+(sigma_e)**2.d0)
              ...
              !
              if (e+ekk.gt.ekq.and.ekq.gt.ekk) then
                 !
!                 reselfen_w_2(ipts)=reselfen_w_2(ipts)+g2*(e-wqv-(ekq-ekk))/((e-wqv-(ekq-ekk))**2.d0+(sigma_e)**2.d0)
                 !
!                 imselfen_w_2(ipts)=imselfen_w_2(ipts)+g2*sigma_e/((e-wqv-(ekq-ekk))**2.d0+(sigma_e)**2.d0)
                 imselfen_w_2(ipts)=imselfen_w_2(ipts)+g2*pi*exp(-((e-wqv-(ekq-ekk))/sigma_e)**2.d0)/(sigma_e*sqrt(pi))
                 !
              elseif (e+ekk.lt.ekq.and.ekq.lt.ekk) then
                 !
!                 reselfen_w_2(ipts)=reselfen_w_2(ipts)+g2*(e+wqv-(ekq-ekk))/((e+wqv-(ekq-ekk))**2.d0+(sigma_e)**2.d0)
                 !
!                 imselfen_w_2(ipts)=imselfen_w_2(ipts)+g2*sigma_e/((e+wqv-(ekq-ekk))**2.d0+(sigma_e)**2.d0)
  ...
  enddo !iq
  !
  reselfen_w(:)=reselfen_w(:)/nqfmesh
  imselfen_w(:)=imselfen_w(:)/nqfmesh
  reselfen_w_2(:)=reselfen_w_2(:)/nqfmesh
  imselfen_w_2(:)=imselfen_w_2(:)/nqfmesh
  !
  do ipts=1,epts
     !
     write(1314,'(100f16.8)') e_min+d_e*(ipts-1)+ekk-ef,reselfen_w(ipts),imselfen_w(ipts)
     write(1315,'(100f16.8)') e_min+d_e*(ipts-1)+ekk-ef,reselfen_w_2(ipts),imselfen_w_2(ipts)
     !
  enddo !ipts
