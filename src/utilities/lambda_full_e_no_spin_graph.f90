program eliashberg_full
  ...
  !energy
  integer :: epts,jpts,ik_irr_cont,ik_irr_2,ik_cont,ik_2
  real(dp) :: e_min,e_max,d_e,e,dgausse
  real(dp),allocatable :: hlambda(:,:)
...
!         do jbnd=1,num_wann
!            do imode=1,3*nat
!               write(1789,'(a,i3,a,i3,a,i3,a,i3,a,i3)')'q= ',iq,' k= ',ik,' ib= ',ibnd,' jb= ',jbnd,' imode= ',imode
!               write(1789,'(100G16.6)') aW_rot_ep_mat_el(iq,ikpt_k,ibnd,jbnd,1,1,imode)
!               write(1789,*) ' '
!            enddo
!         enddo
!      enddo
...
!enddo
  ...
  w_min=0.0d0
  w_max=0.240d0
  wpts=2401 ! how many points for our w variable in eliashberg function a^2F(w)
  !
  ...
  allocate(hlambda(epts,wpts))
  !
  hlambda(:,:)=0.d0
  !
  do ipts=nkpeio3*40-39,nkpeio3*40
     !
     e=e_min+(ipts-1)*d_e
     !
     write(*,*) 'e = ',e,' | ',ipts,'/',epts,' '
     ...
     do ik=1,nkf_irr
        ...
        do jbnd=1,num_wann
           ...
           !
           if (abs(e-ekk-trshld).gt.4.d0*sigma_e) then
              !
           else
              !
              ik_cont=ik_cont+1
              list_ik(ik_cont,1)=ik
              list_ik(ik_cont,2)=jbnd
              !
           endif !abs(e-ekk)
           !
        enddo !jbnd
     ...
     do ik_2=1,ik_cont
        !
        if ((ik_2/1)*1==ik_2) then
           write(*,*) 'ik',ik_2,'/',ik_cont
        endif
        !
        ik=list_ik(ik_2,1)
        jbnd=list_ik(ik_2,2)
        ...
        !
        dgausse=exp(-((e-ekk-trshld)**2.d0)/(sigma_e**2.d0))/(sigma_e*sqrt(pi))
        !
        hlambda(ipts,:)=hlambda(ipts,:)+dgausse*eliash_fminus(:)*weightf_irr(ik)
        !
     enddo !ik_irr
     !
  enddo !epts
  !
  hlambda(:,:)=hlambda(:,:)/nkfmesh
  !
  ...
  do ipts=nkpeio3*40-39,nkpeio3*40
     do jpts=1,wpts
        !
        write(1000+nkpeio3) hlambda(ipts,jpts)
        !
     enddo !jpts
  enddo !ipts
  !
