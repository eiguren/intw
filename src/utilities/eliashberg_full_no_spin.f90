program eliashberg_full_no_spin
...
!THIS IS FOR AVERAGE PROGRAM
  ...
  !=========================================================================================
  allocate(g_Rp(nrr_q,num_wann,num_wann,3*nat))
  ...
  !
  if (lspinorb) then
     trshld=-2.88073485d0 !TlSi
!     trshld=1.9594 !C graph 
  else
     trshld=-2.60126050d0 !TlSi
!     trshld=1.9594 !C graph
  endif
  !
  w_min=0.0d0
!  w_max=0.220d0
  w_max=0.065d0
!  wpts=2201 ! how many points for our w variable in eliashberg function a^2F(w)
  wpts=3251
  !
...
!        jbnd=2
        ...
        dkvec(3)=0.000d0
        !
        kpoint(1)=0.500d0-dkvec(1)*(ik-1)
        kpoint(2)=0.000d0+dkvec(2)*(ik-1)
        kpoint(3)=0.000d0+dkvec(3)*(ik-1)
!        !
!        jbnd=1
!        dkvec(1)=0.0002d0
!        dkvec(2)=0.0002d0
!        dkvec(3)=0.0000d0
!       !
!        kpoint(1)=0.2834d0+dkvec(1)*(ik-1)
!        kpoint(2)=0.2834d0+dkvec(2)*(ik-1)
!        kpoint(3)=0.000d0+dkvec(3)*(ik-1)
!        !
!        if (ik.gt.201) then
!           sigma_e=0.0015d0
!        elseif (ik.gt.151.and.ik.le.201) then
!           sigma_e=0.002d0
!        elseif (ik.gt.101.and.ik.le.151) then
!           sigma_e=0.0025d0
!        elseif (ik.le.101) then
!           sigma_e=0.003d0
!        endif
!        !
     elseif (ik.gt.500.and.ik.lt.835) then
        !
!        jbnd=2
        dkvec(1)=0.001d0
        dkvec(2)=0.001d0
        dkvec(3)=0.000d0
        !
        kpoint(1)=0.000d0+dkvec(1)*(ik-501)
        kpoint(2)=0.000d0+dkvec(2)*(ik-501)
        kpoint(3)=0.000d0+dkvec(3)*(ik-501)
        !
!        jbnd=1
!        dkvec(1)=0.0002d0
!        dkvec(2)=0.0004d0
!        dkvec(3)=0.000d0
       !
!        kpoint(1)=0.3334d0+dkvec(1)*(ik-251)
!        kpoint(2)=0.3334d0-dkvec(2)*(ik-251)
!        kpoint(3)=0.0d0+dkvec(3)*(ik-251)
!        !
!        if (ik.lt.301) then
!            sigma_e=0.0015d0
!        elseif (ik.lt.351.and.ik.ge.301) then
!           sigma_e=0.002d0
!        elseif (ik.lt.401.and.ik.ge.351) then
!           sigma_e=0.0025d0
!        elseif (ik.ge.401) then
!           sigma_e=0.003d0
!        endif
!        !
     elseif (ik.gt.834) then
        !
        dkvec(1)=0.001d0
        dkvec(2)=-0.002d0
        dkvec(3)=0.000d0
        !
        kpoint(1)=0.334d0+dkvec(1)*(ik-835)
        kpoint(2)=0.332d0+dkvec(2)*(ik-835)
        kpoint(3)=0.000d0+dkvec(3)*(ik-835)
        !
     endif
     ...
     write(1314,'(i10,3f8.5,f12.6,100f15.8)') ik,kpoint(:), &
           sqrt(kpoint_cart(1)**2+kpoint_cart(2)**2+kpoint_cart(3)**2),eigen(1,ikpt_k)-2.39934425
!     if (ik.lt.501) then
!        !
!        write(1314,'(i10,3f8.5,f12.6,100f15.8)') ik,kpoint(:), &
!           -sqrt(kpoint_cart(1)**2+kpoint_cart(2)**2+kpoint_cart(3)**2),eigen(:,ikpt_k)-2.57509105
!        !
!     elseif (ik.gt.500.and.ik.lt.834) then
!        !
!        write(1314,'(i10,3f8.5,f12.6,100f15.8)') ik,kpoint(:), &
!           sqrt(kpoint_cart(1)**2+kpoint_cart(2)**2+kpoint_cart(3)**2),eigen(:,ikpt_k)-2.57509105
!        !
!     elseif (ik.gt.833) then
!        !
!        write(1314,'(i10,3f8.5,f12.6,100f15.8)') ik,kpoint(:), &
!           1.d0-sqrt(kpoint_cart(1)**2+kpoint_cart(2)**2+kpoint_cart(3)**2-1.d0/3.d0),eigen(:,ikpt_k)-2.57509105
!        !
!     endif
     !
     ...
     !
     g_Rp(:,:,:,:)=cmplx_0
     call G_ReRpw2kRpw_no_spin(nrr_k,nrr_q,irr_k,ndegen_k,kpoint,g_ReRp,g_Rp)
     !
     ...
     do iq_2=1,nqfmesh_2
        ...
        g_wannier(:,:,:)=cmplx_0
        g_elph(:,:,:)=cmplx_0
        call G_kRpw2kqw_no_spin(nrr_q,irr_q,ndegen_q,qpoint,g_Rp,g_wannier)
        call G_kqw2kqb_no_spin(U_el(:,:,ikpt_kq),U_el(:,:,ikpt_k),U_phon(:,:),g_wannier,g_elph)
        !
        g_me(:)=g_elph(ibnd,jbnd,:)
        !
        call add_q_m_nu_term_full_no_spin(w_min,w_max,wpts,sigma_w,sigma_e,ekk,ekq,omega_phon,g_me,eliash_fplus,eliash_fminus,3*nat-3)
        !
     enddo !iq_2
     !
...
end program eliashberg_full_no_spin
