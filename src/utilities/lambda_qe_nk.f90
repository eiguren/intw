program lambda_qe_nk
  ...
  real(dp),allocatable :: eliash_f(:),eliash_f_so(:,:,:)
  ...
  w_min=0.0d0
  w_max=0.010d0
  wpts=501 ! how many points for our w variable in eliashberg function a^2F(w)
  !
  ...
  if (lspinorb) then
     !
     allocate(eliash_f_so(wpts,nspin,nspin))
  ...
  if (lspinorb) then
     ...
     open(unit=lambda_so_unit,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/lambda_so.dat')))
     !
  endif !lspinorb
  !
  lambda_unit=find_free_unit()
  open(unit=lambda_unit,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/lambda.dat')))
  !
  do ik=63,136
     !
     ...
     if (lspinorb) then
        ...
        do ifs=1,nfs
           ...
           !
           if (ik<10) then
              open(unit=10*ik,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/')//trim('eliash_so_')//trim(ik_loc1)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<100) then
              open(unit=10*ik,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/')//trim('eliash_so_')//trim(ik_loc2)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<1000) then
              open(unit=10*ik,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/')//trim('eliash_so_')//trim(ik_loc3)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           elseif (ik<10000) then
              open(unit=10*ik,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/')//trim('eliash_so_')//trim(ik_loc4)//trim('_')//trim(ifs_loc)//trim('.bin')),form='unformatted')
           endif
           !
           do ipts=1,wpts
              !
              if (ifs==1) then
                 !
                 read(10*ik) eliash_f_so(ipts,1,1)
                 !
              elseif (ifs==2) then
                 !
                 read(10*ik) eliash_f_so(ipts,1,2)
                 !
              elseif (ifs==3) then
                 !
                 read(10*ik) eliash_f_so(ipts,2,1)
                 !
              elseif (ifs==4) then
                 !
                 read(10*ik) eliash_f_so(ipts,2,2)
                 !
              endif !ifs
              !
           enddo !ipts
        ...
        enddo !ifs
        !
        call lambda_calculate_qe_t0_so(wpts,w_min,w_max,d_w,eliash_f_so,lambda_so)
        write(lambda_so_unit,'(i4,100f10.3)') ik,(lambda_so(ifs),ifs=1,nfs)
        !
     endif !lspinorb
     !
     if (ik<10) then
        open(unit=10*ik,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/')//trim('eliash_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
     elseif (ik<100) then
        open(unit=10*ik,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/')//trim('eliash_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
     elseif (ik<1000) then
        open(unit=10*ik,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/')//trim('eliash_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
     elseif (ik<10000) then
        open(unit=10*ik,file=trim(trim(mesh_dir)//trim('eliashberg_qe_3_K_Tl/')//trim('eliash_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
     endif
     !
     do ipts=1,wpts
        !
        read(10*ik) eliash_f(ipts)
        !
     enddo !ipts
     !
     close(10*ik)
     !
     call lambda_calculate_qe_t0(wpts,w_min,w_max,d_w,eliash_f,lambda)
     write(lambda_unit,'(i4,100f10.3)') ik,lambda
     !
  enddo !nkk
  !
