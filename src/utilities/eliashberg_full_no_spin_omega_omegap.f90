program eliashberg_full_no_spin_omega_omegap
  ...
  allocate(g_me(3*nat))
  allocate(g_q(nqfmesh,num_wann,3*nat))
  !
  !==========================================================================
...
!     trshld=-2.60126050d0 !TlSi
  ...
  allocate(eliash_fplus_omega(epts,wpts))
  allocate(eliash_fminus_omega(epts,wpts))
  !
  ...
  ! We know:
  !
  ! a^2F+/-(k,n,si,sj,w,w')=Sum_q_m_nu[|g(q,k,m,n,si,sj,nu)|^2*Delta[w'-w(q,nu)]*Delta[w+E(k,n)-E(k+q,m)+/-w']]
  !
  ...
  ! eigenvalues as: |w+E(k,n)-E(k+q,m)|>4*sigma, we don't take them into account, and then we ease up
  ...
  do ik=1,1
     ...
     do iq=1,nqfmesh
        ...
        do ibnd=1,num_wann
           do imode=1,3*nat
              !
              g_q(iq,ibnd,imode)=g_elph(ibnd,jbnd,imode)
              !
           enddo !imode
        enddo !ibnd
        !
     ...
     do ipts=1,epts
        ...
        do iq=1,nqfmesh
           !
           if ((iq/10000)*10000==iq) then
              write(*,*) 'iq = ',iq,'/',nqfmesh,' ','ipts = ',ipts,'/',epts
           endif 
           !
           qpoint(:)=qfmesh(:,iq)
           call find_k_1BZ_and_G(kpoint+qpoint,nkf1,nkf2,nkf3,i,j,k,kqpoint_in_1bz,GKQ_bz)
           ...
           do ibnd=3,4
              ...
              !
              if (abs(e+ekk-ekq).gt.w_max) then
                 !
              else
                 !
                 iq_cont=iq_cont+1
        ...
        do iq_2=1,nqfmesh_2
           !
           if ((iq_2/5000)*5000==iq_2) then
              write(*,*) 'iq = ',iq_2,'/',nqfmesh_2,' ik = ',ik,'/',nkk,' ipts = ','/',epts
           endif
           !
           ...
           g_me(:)=g_q(iq,ibnd,:)
           !
           call add_q_m_nu_term_full_no_spin(w_min,w_max,wpts,sigma_w,sigma_e,ekk,ekq,omega_phon,g_me,eliash_fplus,eliash_fminus,3*nat)
           !
        enddo !iq_2
        !
        ...
        !
        eliash_fplus_omega(ipts,:)=eliash_fplus(:)
        eliash_fminus_omega(ipts,:)=eliash_fminus(:)
        !
     enddo !ipts
     !
     ...
     !
     if (ik<10) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_w_wp_')//trim(ik_loc1)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_w_wp_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_w_wp_')//trim(ik_loc1)//trim('.dat')))
        open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_w_wp_')//trim(ik_loc1)//trim('.bin')),form='unformatted')
     elseif (ik<100) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_w_wp_')//trim(ik_loc2)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_w_wp_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_w_wp_')//trim(ik_loc2)//trim('.dat')))
        open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_w_wp_')//trim(ik_loc2)//trim('.bin')),form='unformatted')
     elseif (ik<1000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_w_wp_')//trim(ik_loc3)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_w_wp_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_w_wp_')//trim(ik_loc3)//trim('.dat')))
        open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_w_wp_')//trim(ik_loc3)//trim('.bin')),form='unformatted')
     elseif (ik<10000) then
        open(unit=10*ik+1,file=trim(trim(mesh_dir)//trim('eliash_plus_w_wp_')//trim(ik_loc4)//trim('.dat')))
        open(unit=10*ik+100000+1,file=trim(trim(mesh_dir)//trim('eliash_plus_w_wp_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
        open(unit=10*ik+2,file=trim(trim(mesh_dir)//trim('eliash_minus_w_wp_')//trim(ik_loc4)//trim('.dat')))
        open(unit=10*ik+100000+2,file=trim(trim(mesh_dir)//trim('eliash_minus_w_wp_')//trim(ik_loc4)//trim('.bin')),form='unformatted')
     endif
     !
     do ipts=1,epts
        do jpts=1,wpts
           !
           write(10*ik+1,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fplus_omega(ipts,jpts)
           write(10*ik+100000+1) eliash_fplus_omega(ipts,jpts)
           write(10*ik+2,'(100f16.8)') w_min+abs(w_max-w_min)*(ipts-1)/(wpts-1),eliash_fminus_omega(ipts,jpts)
           write(10*ik+100000+2) eliash_fminus_omega(ipts,jpts)
           !
        enddo !jpts
     enddo !ipts
     !
...
end program eliashberg_full_no_spin_omega_omegap
