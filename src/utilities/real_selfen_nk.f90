program real_selfen
  ...
  real(dp) :: trshld,e_min,e_max,e_value,wqv,w_max
  real(dp) :: ekq,ekk,d_e,sigma_w,e_now
  real(dp) :: realse,ef
  integer :: iq_cont_plus,iq_cont_minus,iq_2,nqfmesh_plus,nqfmesh_minus
...
!THIS IS FOR AVERAGE PROGRAM
  ...
  e_max=0.070d0
  nepts=2801
  d_e=abs(e_max-e_min)/(nepts-1)
  !
  allocate(list_iq_plus(nqfmesh*num_wann*3*nat,3))
  allocate(list_iq_minus(nqfmesh*num_wann*3*nat,3))
  !
  kpoint(1)=0.333d0
  kpoint(2)=0.333d0
  kpoint(3)=0.d0
  ...
  do iepts=1,nepts
     !
     write(*,*) 'ie = ',iepts,'/',nepts
     !
     e_value=e_min+d_e*(iepts-1)
     e_now=e_value+ef
     !
     realse=0.0d0
     !
...
!        do ibnd=1,num_wann
           ...
           !
           if (abs(e_now-ekq).gt.w_max) then
              !
           else
              !
              if (iwant) then
              ...
              do imode=1,3*nat-4
                 ...
                 !
                 if (abs(e_now-ekq+wqv).le.4*sigma_e.and.ekq.gt.e_now.and.ekq.lt.ef) then
                    !
                    iq_cont_plus=iq_cont_plus+1
                    list_iq_plus(iq_cont_plus,1)=iq
                    list_iq_plus(iq_cont_plus,2)=ibnd
                    list_iq_plus(iq_cont_plus,3)=imode
                    !
                 endif
                 !
                 if (abs(e_now-ekq-wqv).le.4*sigma_e.and.ekq.lt.e_now.and.ekq.gt.ef) then
                    !
                    iq_cont_minus=iq_cont_minus+1
                    list_iq_minus(iq_cont_minus,1)=iq
                    list_iq_minus(iq_cont_minus,2)=ibnd
                    list_iq_minus(iq_cont_minus,3)=imode
                    !
                 endif
                 !
              enddo !imode
     ...
     enddo !iq
     !
     nqfmesh_plus=iq_cont_plus !how many states (q,m) we take into account in the addition
     nqfmesh_minus=iq_cont_minus
     !
     do iq_2=1,nqfmesh_plus
        !
        if ((iq_2/500)*500==iq_2) then
           write(*,*) 'iq = ',iq_2,'/',nqfmesh_plus,' ik = ',iepts,'/',nepts
        endif
        !
        iq=list_iq_plus(iq_2,1)
        ibnd=list_iq_plus(iq_2,2)
        imode=list_iq_plus(iq_2,3)
        !
        qpoint(:)=qfmesh(:,iq)
        call find_k_1BZ_and_G(kpoint+qpoint,nkf1,nkf2,nkf3,i,j,k,kqpoint_in_1bz,GKQ_bz)
        call switch_indices(nkf1,nkf2,nkf3,ikpt_kq,i,j,k,+1)
        ...
        wqv=omega_phon(imode)
        !
        call add_q_m_nu_real_plus(sigma_w,sigma_e,e_now,ekq,wqv,g_elph(ibnd,jbnd,:,:,imode),termplus)
        !
        realse=realse+termplus
        !
     enddo !iq_plus
     !
     do iq_2=1,nqfmesh_minus
        !
        if ((iq_2/500)*500==iq_2) then
           write(*,*) 'iq = ',iq_2,'/',nqfmesh_minus,' ik = ',iepts,'/',nepts
        endif
        !
        iq=list_iq_minus(iq_2,1)
        ibnd=list_iq_minus(iq_2,2)
        imode=list_iq_minus(iq_2,3)
        !
        qpoint(:)=qfmesh(:,iq)
        call find_k_1BZ_and_G(kpoint+qpoint,nkf1,nkf2,nkf3,i,j,k,kqpoint_in_1bz,GKQ_bz)
        call switch_indices(nkf1,nkf2,nkf3,ikpt_kq,i,j,k,+1)
        ...
        wqv=omega_phon(imode)
        !
        call add_q_m_nu_real_minus(sigma_w,sigma_e,e_now,ekq,wqv,g_elph(ibnd,jbnd,:,:,imode),termminus)
        !
        realse=realse+termminus
        !
     enddo !iq_minus
     !
     ! We normalized the terms
     !
     realse=realse/nqfmesh
     !
     write(2789,'(f12.6,es20.6)') e_value,realse
     !
  enddo !ik

end program real_selfen
