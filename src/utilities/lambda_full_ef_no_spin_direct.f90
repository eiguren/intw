...
!     ef=7.7878d0 !Al bulk
  ...
  !
  allocate(list_ik(nkfmesh*num_wann,2))
  ...
  do ik=1,nkfmesh
     ...
     do ibnd=1,num_wann
        ...
        if (abs(ekk-ef).gt.4.d0*sigma_e) then
           ...
           dosef=dosef+exp(-((ekk-ef)**2.d0)/(sigma_e**2.d0))/(sigma_e*sqrt(pi))
           !
        endif !abs(ekk-ef)
        !
     enddo !ibnd
...
!        endif
        ...
        call G_kqw2kqb_no_spin(U_el(:,:,ikpt_kq),U_el(:,:,ik),U_phon(:,:)/sqrt(amutory),g_wannier,g_elph)
        !
        ...
        do imode=1,3*nat
           ...
           !
           if (abs(ekk-ekq).gt.(4*sigma_e+wqv)) then
              !
           else
              !
              lambda=lambda+(g2/wqv)*(exp(-((ekk-ef)/sigma_e)**2.d0)/(sigma_e*sqrt(pi)))* &
                      (exp(-((ekk-ekq+wqv)/sigma_e)**2.d0)/(sigma_e*sqrt(pi))+exp(-((ekk-ekq-wqv)/sigma_e)**2.d0)/(sigma_e*sqrt(pi)))
           endif
           !
        enddo !imode
     enddo !iq_2
