...
MODULE intw_becmod
  ...
  COMPLEX(DP), ALLOCATABLE ::  &
   ...
   ! LOGICAL, PARAMETER :: gamma_only=.false.
  !
  INTERFACE calbec
     !
...
CONTAINS
  ...
  SUBROUTINE calbec_gamma ( npw, beta, psi, betapsi, nbnd )
    ...
    IF ( npwx < npw ) then
       write(*,*)npwx , npw
       CALL errore ('calbec', 'size mismatch', 2)
    ENDIF
    IF ( present (nbnd) ) THEN
        m = nbnd
    ELSE
    ...
    IF ( nkb /= size (betapsi,1) .or. m > size (betapsi, 2) ) then
      write(*,"(3(a,i4))")"nkb",nkb, "size (betapsi,1)", size (betapsi,1), "size (betapsi,2)", size (betapsi,2)
...
#endif
    IF ( nkb /= size (betapsi,1) .or. m > size (betapsi, 2) ) then
      write(*,"(4(a,i4))")"nkb",nkb, "size (betapsi,1)", size (betapsi,1), "size (betapsi,2)", size (betapsi,2), "m", m
