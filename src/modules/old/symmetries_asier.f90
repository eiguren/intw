...
!          end if
  ...
  subroutine find_the_irreducible_k_set_and_equiv (nspt, k_entire, k_irr, nk_irr, equiv)!, symlink, sym_G)
    ...
    real(dp),intent(in)  :: k_entire (3,nspt)

    ! output
    integer , intent(out)  :: nk_irr
    real(dp), intent(out) :: k_irr  (3, nspt) 
    integer , intent(out) :: equiv(nspt) ! which is the equivalent point
