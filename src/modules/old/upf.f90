...
#define TRIM(a) trim(adjustl(a))
      !
      USE kinds,        ONLY: DP
      USE intw_pseudo_types, ONLY: pseudo_upf, deallocate_pseudo_upf
      USE iotk_module
      !
      USE intw_read_upf_v1_module
      USE intw_read_upf_v2_module
      USE intw_write_upf_v2_module
      !
      IMPLICIT NONE
      PUBLIC
      !
...
SUBROUTINE read_upf(upf, grid, ierr, unit, filename)             !
   ...
   !
   USE intw_radial_grids, ONLY: radial_grid_type, deallocate_radial_grid
!haritz
!   USE intw_read_upf_v1_module,ONLY: read_upf_v1
!haritz
