...
integer, parameter, public :: i8=selected_int_kind(15)
...
public :: tetrahedrize, opside

integer ( kind = i8 ), parameter, public :: bf_max = 800000
integer ( kind = i8 ), parameter, public :: fc_max = 4000000

integer ( kind = i8 ), public:: arg_num
integer ( kind = i8 ), dimension (1:3,1:bf_max), public :: bf
integer ( kind = i8 ), public:: bf_num
integer ( kind = i8 ), public:: face_num
integer ( kind = i8 ), dimension (1:7,1:fc_max), public :: fc
integer ( kind = i8 ) fc_num

integer, public :: io_unit_t=27172



!integer ( kind = i8 ), public::  tetra_num
!integer ( kind = i8 ), public::  tetra_num2
!integer ( kind = i8 ), allocatable, dimension ( :, : ), public :: tetra_node
integer ( kind = i8 ), allocatable, dimension ( : ), public :: vm

contains


subroutine tetrahedrize (node_num, dim_num, node_xyz, tetra_num, tetra_node )
...
!  Parameters:
!
!    Local, integer ( kind = i8 ), parameter BF_MAX, the maximum number of 
...
!
!    Local, integer ( kind = i8 ), parameter FC_MAX, the maximum number of faces.
!    I don't know a reasonable formula for this quantity.
!    If it's not large enough, the program will print a warning message.
!
  implicit none

integer ( kind = i8 ), parameter :: ntetra_max = 100000

integer ( kind = i8 ) ::  tetra_num
integer ( kind = i8 ) ::  tetra_num2
integer ( kind = i8 ), dimension ( 1:4, ntetra_max ) :: tetra_node
!integer ( kind = i8 ), allocatable, dimension ( : )  :: vm


 integer ( kind = i8 ) :: node_num
 integer ( kind = i8 ) :: dim_num
 real ( kind = dp )  ::  node_xyz(1:dim_num,1:node_num) 

 integer ( kind = i8 ), allocatable, dimension ( : ) :: ht
 integer ( kind = i8 ) ht_num
 integer ( kind = i8 ) i
 integer ( kind = 4 ) iarg
 integer ( kind = i8 ) iargc
 integer ( kind = i8 ) :: ierror = 0
 character ( len = 255 ) :: node_filename = ' '
 character ( len = 255 ) :: element_filename = ' '

 !call timestamp ( )

  print*, "node_num, dim_num, ", node_num, dim_num
  
  open(unit=io_unit_t,file='tetrahedra.out',status='unknown')
...
!  call r8mat_header_read (  node_filename, dim_num, node_num )
  ...
  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a)' ) '  Read the header of "' // trim ( node_filename ) //'".'
  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a,i8)' ) '  Spatial dimension DIM_NUM = ', dim_num
  write ( io_unit_t , '(a,i8)' ) '  Number of points NODE_NUM = ', node_num
...
!  allocate ( node_xyz(1:dim_num,1:node_num) )
  ...
  !write ( io_unit_t , '(a)' ) ' '
  !write ( io_unit_t , '(a)' ) '  Read the data in "' // trim ( node_filename ) //'".'

  !call r8mat_transpose_print_some ( dim_num, node_num, node_xyz, 1_i8, 1_i8, 5_i8, 5_i8, &
...
!
 ...
 !do i=1,fc_num
 ! if (fc(5, i)<=0) then 
 ! write(111,"(7(f12.6,x))")node_xyz(1:3,vm(fc(1,i)))
 ! write(111,"(7(f12.6,x))")node_xyz(1:3,vm(fc(2,i)))
 ! write(111,"(7(f12.6,x))")node_xyz(1:3,vm(fc(3,i)))
 ! end if
 !enddo

  if ( ierror /= 0 ) then
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'TABLE_TET_MESH - Fatal error!'
    write ( io_unit_t , '(a,i8)' ) '  DTRIS3 returned IERROR = ', ierror
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'TABLE_TET_MESH'
    write ( io_unit_t , '(a)' ) '  ABNORMAL end of execution!'
    stop
  end if

  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a,i8)' ) '  BF_MAX = ', bf_max
  write ( io_unit_t , '(a,i8)' ) '  BF_NUM = ', bf_num
  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a,i8)' ) '  FC_MAX = ', fc_max
  write ( io_unit_t , '(a,i8)' ) '  FC_NUM = ', fc_num
  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a,i8)' ) '  HT_NUM = ', ht_num
  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a,i8)' ) '  TETRA_NUM = ', tetra_num
  ...
  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a,i8)' ) '  TETRA_NUM2 = ', tetra_num2
!
!  Print a portion.
!
  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a)' ) '  Computed the tet mesh.'
 
  call i4mat_transpose_print_some ( 4_i8, tetra_num, tetra_node, 1_i8, 1_i8, &
    4_i8, 5_i8, '  4 by 5 portion of tetra data:' )
!
!  Write the tet mesh to a file.
!
  call i4mat_write ( element_filename, 4_i8, tetra_num, tetra_node )

  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a)' ) '  Wrote the tetra data to "' &
...
!
  write ( io_unit_t , '(a)' ) ' '
  write ( io_unit_t , '(a)' ) 'TABLE_TET_MESH'
  write ( io_unit_t , '(a)' ) '  Normal end of execution.'
...
!  Parameters:
!
!    Input/output, integer ( kind = i8 ) HDAVFC, head pointer of available
!    records in FC.
!
!    Input/output, integer ( kind = i8 ) FC_NUM, current number of records 
!    used in FC.
!
!    Input, integer ( kind = i8 ) FC_MAX, the maximum number of records 
!    available in FC.
!
!    Input, integer ( kind = i8 ) FC(1:7,1:*), array of face records; see 
!    routine DTRIS3.
!
!    Output, integer ( kind = i8 ) IND, the index of available record (if FC 
!    not full).
!
!    Output, integer ( kind = i8 ) IERR, error flag, which is zero unless an 
!    error occurred.
!
use kinds_t
  implicit none

  integer ( kind = i8 ) fc(7,*)
  integer ( kind = i8 ) fc_num
  integer ( kind = i8 ) hdavfc
  integer ( kind = i8 ) ierr
  integer ( kind = i8 ) ind
  integer ( kind = i8 ) fc_max
  ...
  if ( hdavfc /= 0 ) then
    ...
    ierr = 11
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'AVAILF - Fatal error!'
    write ( io_unit_t , '(a)' ) '  Memory requirements for array FC exceed the'
    write ( io_unit_t , '(a,i12)' ) '  current limit of FC_MAX = ', fc_max
  else
    fc_num = fc_num + 1
    ind = fc_num
  end if
...
!  Parameters:
!
!    Input, real ( kind = dp ) A(1:3), B(1:3), C(1:3), D(1:3), 4 vertices
!    of tetrahedron.
!
!    Input, real ( kind = dp ) E(1:3), fifth point for which 
!    barycentric coordinates found
!
!    Output, real ( kind = dp ) ALPHA(1:4), the scaled barycentric coordinates
...
!
!    Input, real ( kind = dp ) A(1:3), B(1:3), C(1:3), D(1:3), vertices 
!    of tetrahedron.
!
!    Input, real ( kind = dp ) E(1:3), a fifth point; referenced if 
!    and only if INTEST is TRUE.
!
!    Output, real ( kind = dp ) CENTER(1:3), center of sphere; undefined 
!    if A,B,C,D coplanar.
!
!    Output, real ( kind = dp ) RADSQ, the square of radius of sphere; 
!    -1 if A,B,C,D coplanar.
!
!    Output, integer ( kind = i8 ) IN, contains following value if INTEST 
!    is .TRUE.:
...
!    are legal.
!
!    Output, integer ( kind = i8 ) DIGIT, the corresponding value.  If C was
!    'illegal', then DIGIT is -1.
!
  implicit none

  character c
  integer ( kind = i8 ) digit
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) K, the dimension of points.
!
!    Input, integer ( kind = i8 ) N, the number of points.
!
!    Input, integer ( kind = i8 ) LDA, the leading dimension of array A in 
!    calling routine; K <= LDA.
!
!    Input, real ( kind = dp ) A(1:K,1:*), array of points.
!
!    Input/output, integer ( kind = i8 ) MAP(1:N).  On input, the points of A 
...
!
  ...
  do i = n, 2, -1
    ...
    map(1) = map(i)
    map(i) = t
    call dsftdw ( 1_i8, i-1_i8, k, lda, a, map )
  end do
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) K, dimension of points.
!
!    Input, real ( kind = dp ) P(1:K), Q(1:K), two points.
!
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) L, U, the lower and upper index of
!    part of heap.
!
!    Input, integer ( kind = i8 ) K, the dimension of points.
!
...
!    calling routine.
!
!    Input, real ( kind = dp ) A(1:K,1:*), see routine DHPSRT.
!
!    Input/output, integer ( kind = i8 ) MAP(1:*), see routine DHPSRT.
!
  implicit none
...
!    NPT.
!
!    Output, integer ( kind = i8 ) BF_NUM, the number of positions used in BF.
!    BF_NUM <= BF_MAX.
!
!    Output, integer ( kind = i8 ) FC_NUM, the number of positions used in FC.
!    FC_NUM <= FC_MAX.
!
!    Output, integer ( kind = i8 ) NFACE, the number of faces in triangulation; 
!    NFACE <= FC_NUM.
!
!    Output, integer ( kind = i8 ) NTETRA, the number of tetrahedra in 
!    the triangulation.
!
!    Output, integer ( kind = i8 ) BF(1:3,1:BF_NUM), boundary face records 
...
!
!    Output, integer ( kind = i8 ) FC(1:7,1:FC_NUM), face records which are in 
...
!
!    Output, integer ( kind = i8 ) HT(0:SIZHT-1), a hash table using direct
...
!
  ...
  integer ( kind = i8 ) bf_max
  integer ( kind = i8 ) fc_max
  integer ( kind = i8 ) npt
  integer ( kind = i8 ) sizht

  integer ( kind = i8 ) a
  integer ( kind = i8 ) b
  integer ( kind = i8 ) back
  integer ( kind = i8 ) bf(3,bf_max)
  integer ( kind = i8 ) bfi
  real ( kind = dp ) ctr(3)
  integer ( kind = i8 ) e
  integer ( kind = i8 ) fc(7,fc_max)
  integer ( kind = i8 ) fc_num
  integer ( kind = i8 ) front
  integer ( kind = i8 ) hdavbf
  integer ( kind = i8 ) hdavfc
  integer ( kind = i8 ) ht(0:sizht-1)
  integer ( kind = i8 ) i
  integer ( kind = i8 ) i3
  integer ( kind = i8 ) i4
  integer ( kind = i8 ) ierr
  ...
  integer ( kind = i8 ) j
  integer ( kind = i8 ) k
  integer ( kind = i8 ), parameter :: msglvl = 0
  integer ( kind = i8 ) bf_num
  integer ( kind = i8 ) nface
  integer ( kind = i8 ) ntetra
  integer ( kind = i8 ) op
!!  integer ( kind = i8 ) opside
  integer ( kind = i8 ) ptr
  integer ( kind = i8 ) topnv
  integer ( kind = i8 ) top
  integer ( kind = i8 ) va
  integer ( kind = i8 ) vb
  integer ( kind = i8 ) vc
...
!
  ...
  if ( ierr /= 0 ) then
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'DTRIS3 - Error!'
    write ( io_unit_t , '(a,i8)' ) '  FRSTET returned IERR = ', ierr
    return
  end if
!
...
!
  ...
  call htins ( 1_i8, 1_i8, 2_i8, 3_i8, 4_i8, -1_i8, npt, sizht, fc, ht )
  call htins ( 2_i8, 1_i8, 2_i8, 4_i8, 3_i8, -2_i8, npt, sizht, fc, ht )
  call htins ( 3_i8, 1_i8, 3_i8, 4_i8, 2_i8, -3_i8, npt, sizht, fc, ht )
  call htins ( 4_i8, 2_i8, 3_i8, 4_i8, 1_i8, -4_i8, npt, sizht, fc, ht )

  bf(1:3,1) = (/ 4_i8, 3_i8, 2_i8 /)
  bf(1:3,2) = (/ 4_i8, 3_i8, 1_i8 /)
  bf(1:3,3) = (/ 4_i8, 2_i8, 1_i8 /)
  bf(1:3,4) = (/ 3_i8, 2_i8, 1_i8 /)

  if ( msglvl == 4_i8 ) then
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'DTRIS3:'
    write ( io_unit_t , '(a)' ) '  First tetrahedron:'
    write ( io_unit_t , '(2x,i8,2x,i8,2x,i8,2x,i8)' ) vm(1:4)
    write ( io_unit_t , '(a,i8,a,i8)' ) '  I3 = ', i3, '  I4 = ', i4
  end if
!
...
!
  do i = 5, npt
    ...
    if ( msglvl == 4 ) then
      write ( io_unit_t , '(a,i8,a,i8)' ) '  Step: ', i, '  Vertex: ', vi
    end if
...
!
    do while ( top /= 0 )
      ...
      if ( op == 2 ) then
        write ( io_unit_t , '(a)' ) ' '
        write ( io_unit_t , '(a)' ) 'DTRIS3 - Error!'
        write ( io_unit_t , '(a)' ) '  Unexpected return value from OPSIDE.'
        ierr = 301
        return
      end if
...
!
    ...
    if ( ierr /= 0 ) then
      write ( io_unit_t , '(a)' ) ' '
      write ( io_unit_t , '(a)' ) 'DTRIS3 - Error!'
      write ( io_unit_t , '(a,i8)' ) '  NWTHOU returned IERR = ', ierr
      return
    end if
    ...
    if ( ierr /= 0 ) then
      write ( io_unit_t , '(a)' ) ' '
      write ( io_unit_t , '(a)' ) 'DTRIS3 - Error!'
      write ( io_unit_t , '(a,i8)' ) '  SWAPES returned IERR = ', ierr
      return
    end if
...
!
!    Output, integer ( kind = i8 ) COLUMN_NUM, the number of columns in the file.
!
  implicit none

  integer ( kind = i8 ) column_num
  logical got_one
  character ( len = * ) input_file_name
  integer ( kind = i8 ) input_unit
  integer ( kind = i8 ) ios
...
!
  ...
  if ( ios /= 0 ) then
    column_num = -1
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'FILE_COLUMN_COUNT - Fatal error!'
    write ( io_unit_t , '(a)' ) '  Could not open the file:'
    write ( io_unit_t , '(a)' ) '    ' // trim ( input_file_name )
    return
  end if
!
...
!
  ...
  if ( .not. got_one ) then
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'FILE_COLUMN_COUNT - Warning!'
    write ( io_unit_t , '(a)' ) '  The file does not seem to contain any data.'
    column_num = -1
    return
  end if
...
!
!    Output, integer ( kind = i8 ) S_INDEX_LAST.  0 if SUB does not occur in
...
!
  ...
  integer ( kind = i8 ) i
  integer ( kind = i8 ) j
  integer ( kind = i8 ) llen1
  integer ( kind = i8 ) llen2
  character ( len = * ) s
  integer ( kind = i8 ) s_index_last
  character ( len = * ) sub
...
!
!    Output, integer ( kind = i8 ) I, J, the indices of the first and last 
...
!  integer ( kind = i8 ) s_index_last
  ...
  return
end subroutine file_name_ext_get 
...
!
  ...
  integer ( kind = i8 ) i
  integer ( kind = i8 ) j
  integer ( kind = i8 ) len_max
  integer ( kind = i8 ) len_name
  ...
  return
end subroutine file_name_ext_swap 
...
!
!    Output, integer ( kind = i8 ) ROW_NUM, the number of rows found.
!
  implicit none

  integer ( kind = i8 ) bad_num
  integer ( kind = i8 ) comment_num
  integer ( kind = i8 ) ierror
  character ( len = * ) input_file_name
  integer ( kind = i8 ) input_unit
  integer ( kind = i8 ) ios
  character ( len = 255 ) line
  integer ( kind = i8 ) record_num
  integer ( kind = i8 ) row_num
  ...
  if ( ios /= 0 ) then
    ...
    ierror = 1
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'FILE_ROW_COUNT - Fatal error!'
    write ( io_unit_t , '(a)' ) '  Could not open the input file: ' // &
...
!
!    Input, integer ( kind = i8 ) NV, the number of vertices.
!
!    Input, real ( kind = dp ) VCL(1:3,1:*), the vertex coordinate list.
!
!    Input/output, integer ( kind = i8 ) MAP(1:NV), on input, contains vertex 
...
!
!    Output, integer ( kind = i8 ) I3, I4, the indices such that
!    MAP_in(I3) = MAP_out(3) and MAP_in(I4) = MAP_out(4).
!
!    Output, integer ( kind = i8 ) IERR, error flag, which is zero unless 
!    an error occurred.
!
  implicit none
...
!  Parameters:
!
!    Output, integer ( kind = i8 ) IUNIT, the free unit number.
!
  implicit none

  integer ( kind = i8 ) i
  integer ( kind = i8 ) ios
  integer ( kind = i8 ) iunit
  logical lopen
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) IND, the index of FC array.
!
!    Input, integer ( kind = i8 ) N, the upper bound on vertex indices.
!
!    Input, integer ( kind = i8 ) P, the size of hash table.
!
!    Input/output, integer ( kind = i8 ) FC(1:7,1:*), the array of face records; 
!    see routine DTRIS3.  On output, one link in FC is updated.
!
!    Input/output, integer ( kind = i8 ) HT(0:P-1), the hash table using 
...
!    Input, integer ( kind = i8 ) IND, the index of FC array.
!
!    Input, integer ( kind = i8 ) A, B, C, D, E, the first 5 fields of FC 
!    record (or column).
!
!    Input, integer ( kind = i8 ) N, the upper bound on vertex indices.
!
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) A,B,C, first 3 fields of FC record 
!    (in any order).
!
!    Input, integer ( kind = i8 ) N, upper bound on vertex indices.
!
!    Input, integer ( kind = i8 ) P, size of hash table.
!
!    Input, integer ( kind = i8 ) FC(1:7,1:*), array of face records; 
!    see routine DTRIS3.
!
!    Input, integer ( kind = i8 ) HT(0:P-1), hash table using direct chaining.
!
!    Output, integer ( kind = i8 ) HTSRC, index of FC record with key A,B,C 
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) I, the number to be divided.
!
!    Input, integer ( kind = i8 ) J, the number that divides I.
!
!    Output, integer ( kind = i8 ) I4_MODP, the nonnegative remainder when I is
!    divided by J.
!
  implicit none

  integer ( kind = i8 ) i
  integer ( kind = i8 ) i4_modp
  integer ( kind = i8 ) j

  if ( j == 0 ) then
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'I4_MODP - Fatal error!'
    write ( io_unit_t , '(a,i8)' ) '  I4_MODP ( I, J ) called with J = ', j
    stop
  end if
...
!  Parameters:
!
!    Input/output, integer ( kind = i8 ) I, J.  On output, the values of I and
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) IVAL, an integer value.
!
!    Input, integer ( kind = i8 ) ILO, IHI, the desired bounds.
!
!    Output, integer ( kind = i8 ) I4_WRAP, a "wrapped" version of IVAL.
!
  implicit none

!  integer ( kind = i8 ) i4_modp
  integer ( kind = i8 ) i4_wrap
  integer ( kind = i8 ) ihi
  integer ( kind = i8 ) ilo
  integer ( kind = i8 ) ival
  integer ( kind = i8 ) jhi
  integer ( kind = i8 ) jlo
  integer ( kind = i8 ) wide
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) M, N, the number of rows and columns.
!
!    Input, integer ( kind = i8 ) A(M,N), an M by N matrix to be printed.
!
...
!
  ...
  integer ( kind = i8 ) a(m,n)
  character ( len = * ) title

  call i4mat_transpose_print_some ( m, n, a, 1_i8, 1_i8, m, n, title )
...
!    Input, integer ( kind = i8 ) A(M,N), an M by N matrix to be printed.
!
!    Input, integer ( kind = i8 ) ILO, JLO, the first row and column to print.
!
!    Input, integer ( kind = i8 ) IHI, JHI, the last row and column to print.
!
!    Input, character ( len = * ) TITLE, an optional title.
!
  implicit none

  integer ( kind = i8 ), parameter :: incx = 10
  integer ( kind = i8 ) m
  integer ( kind = i8 ) n
  ...
  integer ( kind = i8 ) i
  integer ( kind = i8 ) i2
  integer ( kind = i8 ) i2hi
  integer ( kind = i8 ) i2lo
  integer ( kind = i8 ) ihi
  integer ( kind = i8 ) ilo
  integer ( kind = i8 ) inc
  integer ( kind = i8 ) j
  integer ( kind = i8 ) j2hi
  integer ( kind = i8 ) j2lo
  integer ( kind = i8 ) jhi
  integer ( kind = i8 ) jlo
  character ( len = * ) title
  ...
  do i2lo = max ( ilo, 1 ), min ( ihi, m ), incx
    ...
    write ( io_unit_t , '(''  Row '',10a7)' ) ctemp(1:inc)
    write ( io_unit_t , '(a)' ) '  Col'
    write ( io_unit_t , '(a)' ) ' '
    ...
    do j = j2lo, j2hi
      ...
      write ( io_unit_t , '(i5,1x,10a7)' ) j, ( ctemp(i), i = 1, inc )
...
!
!    Input, integer ( kind = i8 ) M, the spatial dimension.
!
!    Input, integer ( kind = i8 ) N, the number of points.
!
!    Input, integer ( kind = i8 ) TABLE(M,N), the table data.
!
  implicit none
  ...
  integer ( kind = i8 ) output_status
  integer ( kind = i8 ) output_unit
  character ( len = 30 ) string
  integer ( kind = i8 ) table(m,n)
!
!  Open the file.
!
  ...
  if ( output_status /= 0 ) then
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'I4MAT_WRITE - Fatal error!'
    write ( io_unit_t , '(a,i8)' ) '  Could not open the output file "' // &
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) I, the (local) index of next vertex inserted in
!    triangulation.
!
!    Input, integer ( kind = i8 ) NPT, the number of 3D points to be 
!    triangulated.
!
!    Input, integer ( kind = i8 ) SIZHT, the size of hash table HT.
!
!    Input/output, integer ( kind = i8 ) BF_NUM, the number of positions in BF.
!
!    Input/output, integer ( kind = i8 ) FC_NUM, the number of positions in FC.
!
!    Input, integer ( kind = i8 ) BF_MAX, the maximum size available for BF.
!
!    Input, integer ( kind = i8 ) FC_MAX, the maximum size available for FC.
!
!    Input/output, integer ( kind = i8 ) BF(1:3,1:BF_MAX), the array of 
!    boundary face records; see DTRIS3.
!
!    Input/output, integer ( kind = i8 ) FC(1:7,1:FC_MAX), the array of face 
!    records; see routine DTRIS3.
!
!    Input/output, integer ( kind = i8 ) HT(0:SIZHT-1), the hash table using 
!    direct chaining.
!
!    Input/output, integer ( kind = i8 ) NTETRA, the number of tetrahedra in 
!    triangulation.
!
!    Input/output, integer ( kind = i8 ) HDAVBF, the head pointer to available 
!    BF records.
!
!    Input/output, integer ( kind = i8 ) HDAVFC, the head pointer to available 
!    FC records.
!
!    Input, integer ( kind = i8 ) FRONT, the index of front of queue (or top 
!    of stack) of visible boundary faces.
!
!    Output, integer ( kind = i8 ) BACK, the index of back of queue (or bottom 
!    of stack) of visible boundary faces (which become interior faces).
!
!    Output, integer ( kind = i8 ) BFI, the index of FC of a boundary face 
!    containing vertex I.
!
!    Output, integer ( kind = i8 ) IERR, error flag, which is zero unless an 
!    error occurred.
...
!
  ...
  do
    ...
    do e = 1, 3
      ...
      if ( ierr /= 0 ) then
        write ( io_unit_t , '(a)' ) ' '
        write ( io_unit_t , '(a)' ) 'NWTHOU - Error!'
        write ( io_unit_t , '(a,i8)' ) '  AVAILF returned IERR = ', ierr
        return
      end if
      ...
      if ( fc(7,nbr) /= -1 ) then
        ...
        if ( hdavbf /= 0 ) then
          ...
          if ( bf_max <= bf_num ) then
            write ( io_unit_t , '(a)' ) ' '
            write ( io_unit_t , '(a)' ) 'NWTHOU - Error!'
            write ( io_unit_t , '(a)' ) '  BF_MAX <= BF_NUM.'
            write ( io_unit_t , '(a)' ) '  BF_MAX must be increased to proceed.'
            ierr = 12
            return
          end if
  ...
  do
    ...
    do
      ...
      if ( nbr <= 0 ) then
        write ( io_unit_t , '(a)' ) ' '
        write ( io_unit_t , '(a)' ) 'NWTHOU - Error!'
        write ( io_unit_t , '(a,i8)' ) '  HTSRC returned NBR <= 0.'
        ierr = 300
        return
      end if
...
!  Parameters:
!
!    Input, real ( kind = dp ) A(1:3), B(1:3), C(1:3), D(1:3), E(1:3),
!    five 3D points.
!
!    Output, integer ( kind = i8 ) OPSIDE, the result of the test:
...
!  Parameters:
!
!    Input/output, integer ( kind = i8 ) I, J, K, on output are sorted into
!    nondecreasing order.
!
  implicit none
...
!    Input, integer ( kind = i8 ) N, the number of points.
!
!    Output, real ( kind = dp ) TABLE(M,N), the table data.
!
  implicit none
  ...
  if ( input_status /= 0 ) then
    ierror = 1
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'R8MAT_DATA_READ - Fatal error!'
    write ( io_unit_t , '(a,i8)' ) '  Could not open the input file "' // &
  ...
  do while ( j < n )
    ...
    if ( input_status /= 0 ) then
      write ( io_unit_t , '(a)' ) ' '
      write ( io_unit_t , '(a)' ) 'R8MAT_DATA_READ - Fatal error!'
      write ( io_unit_t , '(a)' ) '  Error while reading lines of data.'
      write ( io_unit_t , '(a,i8)' ) '  Number of values expected per line M = ', m
      write ( io_unit_t , '(a,i8)' ) '  Number of data lines read, J =         ', j
      write ( io_unit_t , '(a,i8)' ) '  Number of data lines needed, N =       ', n
      stop
    end if
...
!    Input, character ( len = * ) INPUT_FILENAME, the name of the input file.
!
!    Output, integer ( kind = i8 ) M, spatial dimension.
!
!    Output, integer ( kind = i8 ) N, the number of points.
!
  implicit none
  ...
  if ( m <= 0 ) then
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'R8MAT_HEADER_READ - Fatal error!'
    write ( io_unit_t , '(a)' ) '  There was some kind of I/O problem while trying'
    write ( io_unit_t , '(a)' ) '  to count the number of data columns in'
    write ( io_unit_t , '(a)' ) '  the file "' // trim ( input_filename ) // '".'
    stop
  end if
  ...
  if ( n <= 0 ) then
    ...
    write ( io_unit_t , '(a)' ) 'R8MAT_HEADER_READ - Fatal error!'
    write ( io_unit_t , '(a)' ) '  There was some kind of I/O problem while trying'
    write ( io_unit_t , '(a)' ) '  to count the number of data rows in'
    write ( io_unit_t , '(a)' ) '  the file "' // trim ( input_filename ) // '".'
    stop
  end if
...
!
  ...
  integer ( kind = i8 ), parameter :: incx = 5
  integer ( kind = i8 ) m
  integer ( kind = i8 ) n
  ...
  do i2lo = max ( ilo, 1 ), min ( ihi, m ), incx
    ...
    write ( io_unit_t , '(''  Row   '',5a14)' ) ctemp(1:inc)
    write ( io_unit_t , '(a)' ) '  Col'
    write ( io_unit_t , '(a)' ) ' '
    ...
    do j = j2lo, j2hi
      ...
      write ( io_unit_t , '(i5,1x,5a14)' ) j, ( ctemp(i), i = 1, inc )
...
!
!    Output, real ( kind = dp ) R, the real value that was read from the string.
!
!    Output, integer ( kind = i8 ) IERROR, error flag.
!
...
!
!    Output, integer ( kind = i8 ) LCHAR, the number of characters read from
...
!  logical ch_eqi
  character c
  integer ( kind = i8 ) ierror
  integer ( kind = i8 ) ihave
  integer ( kind = i8 ) isgn
  integer ( kind = i8 ) iterm
  integer ( kind = i8 ) jbot
  integer ( kind = i8 ) jsgn
  integer ( kind = i8 ) jtop
  integer ( kind = i8 ) lchar
  integer ( kind = i8 ) nchar
  integer ( kind = i8 ) ndig
  real ( kind = dp ) r
  real ( kind = dp ) rbot
  real ( kind = dp ) rexp
  real ( kind = dp ) rtop
...
!
!    Input, integer ( kind = i8 ) N, the number of values expected.
!
!    Output, real ( kind = dp ) RVEC(N), the values read from the string.
!
!    Output, integer ( kind = i8 ) IERROR, error flag.
!    0, no errors occurred.
...
!
!    Output, integer ( kind = i8 ) NWORD, the number of "words" in the string.
...
!
!    Input, integer ( kind = i8 ) NPT, the number of 3D points to 
!    be triangulated.
!
!    Input, integer ( kind = i8 ) SIZHT, the size of hash table HT.
!
!    Input/output, integer ( kind = i8 ) FC_NUM, the number of positions used 
!    in FC array.
!
!    Input, integer ( kind = i8 ) FC_MAX, the maximum size available for FC.
!
!    Input, real ( kind = dp ) VCL(1:3,1:*), the vertex coordinate list.
!
!    Input, integer ( kind = i8 ) VM(1:NPT), the indices of vertices of VCL being
!    triangulated.
!
!    Input/output, integer ( kind = i8 ) BF(1:3,1:*), the  array of boundary 
!    face records; see DTRIS3.
!
!    Input/output, integer ( kind = i8 ) FC(1:7,1:FC_MAX), the array of 
!    face records; see routine DTRIS3.
!
!    Input/output, integer ( kind = i8 ) HT(0:SIZHT-1), the hash table using 
!    direct chaining.
!
!    Input/output, integer ( kind = i8 ) NTETRA, the number of tetrahedra 
!    in triangulation.
!
!    Input/output, integer ( kind = i8 ) HDAVFC, the head pointer to available 
!    FC records.
!
!    Input/output, integer ( kind = i8 ) FRONT, BACK, the indices of front and 
!    back of queue of interior faces for which sphere test is applied.
!
!    Output, integer ( kind = i8 ) IFAC, the index of last face for which sphere
!    test applied, or 0.
!
!    Output, integer ( kind = i8 ) IERR, error flag, which is zero unless an 
!    error occurred.
!
  ...
  integer ( kind = i8 ) b
  integer ( kind = i8 ) back
  integer ( kind = i8 ) bf(3,*)
  integer ( kind = i8 ) bfx(2)
  logical bndcon
  integer ( kind = i8 ) c
  real ( kind = dp ) center(3)
  integer ( kind = i8 ) d
...
!  integer ( kind = i8 ) htsrc
  integer ( kind = i8 ) i
  integer ( kind = i8 ) ierr
  integer ( kind = i8 ) ifac
  integer ( kind = i8 ) in
  integer ( kind = i8 ) ind
  integer ( kind = i8 ) ind1
  integer ( kind = i8 ) ind2
  integer ( kind = i8 ) indx(2)
  integer ( kind = i8 ) j
  integer ( kind = i8 ) k
  integer ( kind = i8 ) kneg
  integer ( kind = i8 ) kzero
  integer ( kind = i8 ), parameter :: msglvl = 0
  integer ( kind = i8 ) nbr(2,2)
  integer ( kind = i8 ) ntetra
  real ( kind = dp ) radsq
  real ( kind = dp ) vcl(3,*)
  integer ( kind = i8 ) va
  ...
  do
    ...
    if ( msglvl == 4 ) then
      write ( io_unit_t ,600) ind,a,b,c,d,e
    end if
    ...
    if ( in == 2 ) then
      write ( io_unit_t , '(a)' ) ' '
      write ( io_unit_t , '(a)' ) 'SWAPES - Fatal error!'
      write ( io_unit_t , '(a)' ) '  CCSPH returned IN = 2.'
      ierr = 301
      return
    end if
    ...
    if ( 1 <= in ) then
      ...
      if ( degen ) then
        write ( io_unit_t , '(a)' ) ' '
        write ( io_unit_t , '(a)' ) 'SWAPES - Fatal error!'
        write ( io_unit_t , '(a)' ) '  BARYTH detected a degenerate tetrahedron.'
        ierr = 301
        return
      else if ( 0.0D+00 < alpha(4) ) then
        write ( io_unit_t , '(a)' ) ' '
        write ( io_unit_t , '(a)' ) 'SWAPES - Fatal error!'
        write ( io_unit_t , '(a)' ) '  BARYTH detected 0 < ALPHA(4).'
        ierr = 309
        return
      end if
...
!
      if ( kneg == 1 .and. kzero == 0 ) then
        ...
        if ( ierr /= 0 ) then
          write ( io_unit_t , '(a)' ) ' '
          write ( io_unit_t , '(a)' ) 'SWAPES - Fatal error!'
          write ( io_unit_t , '(a, i8)' ) '  UPDATF returned IERR = ', ierr
          return
        end if
...
!
        ...
        if ( ind1 <= 0 ) then
          write ( io_unit_t , '(a)' ) ' '
          write ( io_unit_t , '(a)' ) 'SWAPES - Fatal error!'
          write ( io_unit_t , '(a)' ) '  HTSRC returned IND1 <= 0.'
          ierr = 300
          return
        end if
        ...
        if ( fc(4,ind1) == e .or. fc(5,ind1) == e ) then
          ...
          if ( msglvl == 4 ) then
            write ( io_unit_t ,620) c,d,e
          end if

        else

          if ( msglvl == 4 ) then
            write ( io_unit_t ,630) a,b,d,e
          end if
...
!
        ...
        if ( ind2 <= 0 ) then
          write ( io_unit_t , '(a)' ) ' '
          write ( io_unit_t , '(a)' ) 'SWAPES - Fatal error!'
          write ( io_unit_t , '(a)' ) '  HTSRC returned IND2 <= 0.'
          ierr = 300
          return
        end if
        ...
        if ( f <= 0 .and. g <= 0 ) then
          ...
          if ( ind <= 0 ) then
            write ( io_unit_t , '(a)' ) ' '
            write ( io_unit_t , '(a)' ) 'SWAPES - Fatal error!'
            write ( io_unit_t , '(a)' ) '  HTSRC returned IND <= 0.'
            ierr = 300
            return
          end if
          ...
          if ( msglvl == 4 ) then
            write ( io_unit_t ,650) a,b,d,e,f
          end if

        else

          if ( msglvl == 4 ) then
            write ( io_unit_t ,660) a,b,d,e,f,g
          end if
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) FC_MAX, the maximum number of positions 
!    in the FC array.
!
!    Input, integer ( kind = i8 ) FC_NUM, the number of positions used in FC.
!
!    Input, integer ( kind = i8 ) VM(1:*), the indices of vertices of VCL that
!    are triangulated.
!
!    Input, integer ( kind = i8 ) FC(7,FC_MAX), array of face records; 
!    see routine DTRIS3.
!
!    Input, integer ( kind = i8 ) TETRA_NUM, the number of tetrahedrons expected.
!
!    Output, integer ( kind = i8 ) TETRA_NUM2, the number of tetrahedrons found.
!
!    Output, integer ( kind = i8 ) TETRA(4,TETRA_NUM), contains global 
...
!
  ...
  integer ( kind = i8 ) l
  integer ( kind = i8 ) t(4)
  integer ( kind = i8 ) tetra(4,tetra_num)
  integer ( kind = i8 ) tetra_num2
  integer ( kind = i8 ) vm(*)
  ...
  if ( tetra_num2 /= tetra_num ) then
    write ( io_unit_t , '(a)' ) ' '
    write ( io_unit_t , '(a)' ) 'TETLST - Warning!'
    write ( io_unit_t , '(a)' ) '  Inconsistent tetrahedron information.'
    write ( io_unit_t , '(a,i8)' ) '  Was expecting TETRA_NUM = ', tetra_num
    write ( io_unit_t , '(a,i8)' ) '  Found TETRA_NUM2 = ', tetra_num2
    return
  end if
...
!
  ...
  write ( io_unit_t , '(i2,1x,a,1x,i4,2x,i2,a1,i2.2,a1,i2.2,a1,i3.3,1x,a)' ) &
...
!  Parameters:
!
!    Input, integer ( kind = i8 ) A, B, C, the first 3 fields of FC record 
!    (in any order).
!
!    Input, integer ( kind = i8 ) D, E, the fourth vertex indices of old and 
!    new tetrahedrons.
!
!    Input, integer ( kind = i8 ) I, the vertex index determining whether 
!    face put on queue.
!
!    Input, integer ( kind = i8 ) N, the upper bound on vertex indices.
!
!    Input, integer ( kind = i8 ) P, the size of hash table.
!
!    Input/output, integer ( kind = i8 ) FRONT, BACK, the front and back 
!    pointers of queue.
!
!    Input, integer ( kind = i8 ) FC(1:7,1:*), the array of face records; 
!    see routine DTRIS3.
!
!    Input, integer ( kind = i8 ) HT(0:P-1), the hash table using direct 
!    chaining.
!
!    Output, integer ( kind = i8 ) IERR, error flag, which is zero unless 
!    an error occurred.
...
!  Parameters:
!
!    Input, real ( kind = dp ) PT(1:3), the 3D point.
!
!    Input, real ( kind = dp ) CTR(1:3), the 3D point in interior of
!    triangulation.
!
!    Input, real ( kind = dp ) VCL(1:3,1:*), the vertex coordinate list.
!
!    Input, integer ( kind = i8 ) VM(1:*), the indices of vertices of VCL 
!    being triangulated.
!
!    Input, integer ( kind = i8 ) BF(1:3,1:*), the array of boundary face 
!    records; see DTRIS3.
!
!    Input/output, integer ( kind = i8 ) FC(1:7,1:*), the array of face records;
...
!
!    Input/output, integer ( kind = i8 ) TOPV.  On input, index of FC of visible
!    boundary face.  On output, index of top of stack of visible boundary faces.
!
!    Input, integer ( kind = i8 ) TOPNV, the index of top of stack of boundary 
...
!  integer ( kind = i8 ) opside
  real ( kind = dp ) pt(3)
  integer ( kind = i8 ) ptr
  integer ( kind = i8 ) topn
  integer ( kind = i8 ) topnv
  integer ( kind = i8 ) topt
  integer ( kind = i8 ) topv
  integer ( kind = i8 ) va
  integer ( kind = i8 ) vb
  integer ( kind = i8 ) vc
  integer ( kind = i8 ) vm(*)
