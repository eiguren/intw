...
!----------------------------------------------------------------------------!
  ...
  use kinds, only: dp
  use intw_input_parameters, only: read_input, broadcast_input, &
                                   mesh_dir, prefix, TR_symmetry, &
                                   nG_shell_max, number_G_shells, &
                                   intw2W_method, intw2W_fullzone, &
                                   nk1, nk2, nk3, nk1s, nk2s, nk3s
  use intw_utility, only: get_timing, generate_kmesh, find_free_unit, create_or_append_to_file
  use intw_reading, only: read_kpoints_data_file_xml, read_parameters_data_file_xml, &
                          get_ngm, get_nGmax, get_gvec, deallocate_reading_variables, &
                          nkpoints_QE, ng_max, ngm, gvec, nat, tau, atom_labels, &
                          ntyp, ityp, nsym, s, ftau, atom_pfile, nbands
  use intw_useful_constants, only: mpi_rank, mpi_nproc
  use intw_symmetries, only: allocate_symmetry_related_k, deallocate_symmetry_related_k, &
                             allocate_and_build_spin_symmetry_matrices, &
                             find_inverse_symmetry_matrices_indices, &
                             set_symmetry_relations, get_G_shells, &
  ...
  !
  ! variables
  public :: kpoints_QE, kmesh
  !
  ! subroutines
  public :: setup_QE, setup, cleanup, write_input, write_parameters_data_file_xml
  !
  private
  !
  !
...
stop
  ...
  use intw_W90, only: allocate_and_read_W90, produce_u_mesh
