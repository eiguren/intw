      subroutine rotaxis(A,v,ang,inv_used)
      implicit none
      ...
      if (det.lt.0) S=matmul(Inv,S) 
      !I think these is an ambiguity on the sing of u. Thus we compare
      !the reconstructed S and if it does not match the original matrix 
      !we try -u.
      ...
      inv_used=.false.
      if (abs(det+1.0).lt.0.01) inv_used=.true.

      return
      end

      Subroutine diagreal(A,eig,ang,axis,deter)
      implicit none 
      INTEGER          NIN, NOUT
