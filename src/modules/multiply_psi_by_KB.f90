       ...
       USE kinds, only : dp 
       USE intw_uspp_param, ONLY : upf, lmaxkb, nhm, nh
       USE intw_uspp,       ONLY : nkb, vkqb, vkb, nhtol, nhtolm, indv,  dvan
            ...
            do na = 1, nat !
              do ih=1,nh(ityp(na))
                ...
                do ig=1,nG_max
                 ...
                 projec_d1(na,ih,ipol,spol) +    conjg(vkb(iG ,na, ih)) * psi(ig, ibnd,spol)

                 projec_d2(na,ih,ipol,spol) =   &
                 projec_d2(na,ih,ipol,spol) +   ( - tpiba * cmplx_i) * ( k_(ipol)  + gvec_cart(ipol,iGk) ) * conjg(vkb(iG,na, ih)) * &
                                                 psi(ig,ibnd, spol) 
              ...
              do ih=1,nh(ityp(na))
                ...
                do ig=1,nG_max
                 ...
                 dvnl_psi(ig, ibnd,spol,mode) = dvnl_psi(ig,ibnd,spol,mode)&
                    + dvan (ih, ih,ityp(na) ) * projec_d2(na,ih,ipol,spol) * vkqb(iG,na, ih)
                 dvnl_psi(ig, ibnd,spol,mode) = dvnl_psi(ig,ibnd,spol,mode)&
                    + dvan (ih, ih, ityp(na)) * projec_d1(na,ih,ipol,spol) * vkqb(iG,na, ih)* &
                                               (tpiba * cmplx_i)  * (k_(ipol) +  q_(ipol) + gvec_cart(ipol,iGk) )
                enddo
              enddo
           enddo
          enddo
