subroutine dvqpsi_us_only_haritz(nat,nG_max,nbands,npol,igk,wfc_k,dvpsi)
  !------------------------------------------------------------------------------
  !
  !========================================================================
  ...
  use intw_pseudo, only: nkb, vkb, vkqb, dvan, dvan_so
  use intw_pseudo, only: nh
  ...
  !I/O variables
  !
  integer,intent(in) :: nat, nG_max, nbands, npol, igk(ng_max)
  ...
  integer :: nsp, na, mu, ig, igg, nt, ibnd
                ...
                do ipol = 1, 3
                  ...
                  if (abs(uact(mu+1))+abs(uact(mu+2))+abs(uact(mu+3))>eps) then
                    ...
                    if (noncolin) then
                      ...
                      enddo !is
                      !
                    elseif (gamma_only) then ! gamma_only
                      !
                      ps1(ikb,ibnd)=ps1(ikb,ibnd)+dvan(ih,jh,nt)*alphap(ipol,1)%r(jkb,ibnd)* &
                                      uact(mu+ipol)
                      !
                      ps2(ikb,ibnd,ipol)=ps2(ikb,ibnd,ipol)+dvan(ih,jh,nt)*becp1(1)%r(jkb,ibnd)* &
                                            (-cmplx_i*uact(mu+ipol)*tpiba)
                      !
                    else ! general case
                      !
        ...
        if (ok) then
          ...
          do ig=1,nG_max
            ...
            if ((igg>0).and.(igg<nG_max)) then
              !
              aux(ig)=vkqb(ig,ikb)*(gvec_cart(ipol,igg))
              !
            else
              !
              aux(ig)=cmplx_0
  ...
  return
  !
end subroutine dvqpsi_us_only_haritz
