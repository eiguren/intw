...
!
!----------------------------------------------------------------------------
SUBROUTINE gk_sort( k, ngm, g, ecut, ngk, igk)
   !----------------------------------------------------------------------------
   !
   ...
   INTEGER, INTENT(out) :: igk(nG_max) ! the correspondence k+G <-> G
   !
   REAL(DP) :: gk(nG_max) ! the moduli of k+G
   INTEGER :: ng   ! counter on   G vectors
   INTEGER :: nk   ! counter on k+G vectors
   REAL(DP) :: q   ! |k+G|^2
   REAL(DP) :: q2x ! upper bound for |G|
   real(dp),parameter :: eps8=0.00000001
   !
